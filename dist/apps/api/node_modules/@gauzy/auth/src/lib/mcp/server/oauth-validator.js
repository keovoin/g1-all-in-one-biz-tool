...
class OAuthValidator {
    ...
    async getJose() {
        if (!this.josePromise) {
            this.josePromise = Promise.resolve().then(() => __importStar(require('jose'))).catch((err) => {
                this.josePromise = null;
                throw err;
            });
        }
    ...
    async introspectToken(token) {
        ...
        try {
            ...
            if (hasSecret) {
                const credentials = Buffer.from(`${this.config.introspection.clientId}:${this.config.introspection.clientSecret}`).toString('base64');
     ...
     */
    validateAudience(tokenAudience, expectedAudience) {
        if (!tokenAudience) {
            return false;
        }
     ...
     */
    static createAuthorizationError(error, description, scope, errorUri) {
        return {
            error,
     ...
     */
    static formatWWWAuthenticateHeader(resourceMetadataUrl, error) {
...
exports.OAuthValidator = OAuthValidator;
