...
const authorization_config_1 = require("../interfaces/authorization-config");
...
class ConfigManager {
    ...
    getEnvEnvironment(key, defaultEnv) {
        ...
        switch (normalizedValue) {
            ...
            case 'stage':
                ...
                return 'production';
            default:
                this.securityLogger.warn(`Unexpected ${key} value: "${rawValue}". Valid values: production, development, stage. Defaulting to "${defaultEnv}".`);
                return defaultEnv;
        }
    }
    static getInstance() {
    ...
    loadConfiguration() {
        const config = {
            ...
            corsOrigins: this.parseStringArray(this.getEnvString('MCP_CORS_ORIGINS', this.getEnvString('MCP_CORS_ORIGIN', ''))),
            ...
            redisUrl: (() => {
                ...
                console.warn('Redis is enabled but neither REDIS_URL nor REDIS_HOST/REDIS_PORT are configured. Redis will not be used.');
                return undefined;
            })(),
            // OAuth 2.0
    ...
    loadOAuthConfiguration() {
        const config = {
            ...authorization_config_1.DEFAULT_AUTHORIZATION_CONFIG,
            enabled: this.getEnvBoolean(authorization_config_1.AUTH_ENV_KEYS.ENABLED, false),
            resourceUri: this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.RESOURCE_URI, ''),
            authorizationServers: [],
        };
        // Parse authorization servers
        const serversEnv = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.AUTHORIZATION_SERVERS);
        if (serversEnv) {
            try {
        ...
        const scopesEnv = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.REQUIRED_SCOPES);
        if (scopesEnv) {
            config.requiredScopes = this.parseStringArray(scopesEnv);
        }
        // JWT configuration
        if (config.jwt) {
            config.jwt.audience = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.JWT_AUDIENCE);
            config.jwt.issuer = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.JWT_ISSUER);
            config.jwt.publicKey = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.JWT_PUBLIC_KEY);
            config.jwt.jwksUri = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.JWT_JWKS_URI);
            const algorithmsEnv = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.JWT_ALGORITHMS);
            if (algorithmsEnv) {
                config.jwt.algorithms = this.parseStringArray(algorithmsEnv);
            }
        }
        // Introspection configuration
        const introspectionEndpoint = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.INTROSPECTION_ENDPOINT);
        const introspectionClientId = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.INTROSPECTION_CLIENT_ID);
        const introspectionClientSecret = this.getEnvString(authorization_config_1.AUTH_ENV_KEYS.INTROSPECTION_CLIENT_SECRET);
        ...
        // Cache configuration
        if (config.cache) {
            config.cache.tokenTtl = this.getEnvNumber(authorization_config_1.AUTH_ENV_KEYS.TOKEN_CACHE_TTL, config.cache.tokenTtl);
            config.cache.metadataTtl = this.getEnvNumber(authorization_config_1.AUTH_ENV_KEYS.METADATA_CACHE_TTL, config.cache.metadataTtl);
        }
        return config;
    }
    /**
    ...
    validateConfiguration(config) {
        ...
        if (config.oauth.enabled) {
            ...
            if (config.oauth.authorizationServers.length === 0 && !hasIntrospection) {
                errors.push('At least one authorization server or token introspection must be configured when OAuth is enabled');
            }
            if (!hasJwtKeys && !hasIntrospection) {
                errors.push('Either MCP_AUTH_JWT_JWKS_URI or MCP_AUTH_JWT_PUBLIC_KEY must be set for JWT validation, or MCP_AUTH_INTROSPECTION_* must be configured');
            }
        }
    ...
    getRequiredEnvString(key, fallback) {
