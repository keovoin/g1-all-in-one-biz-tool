...
exports.LoginAttemptService = exports.LOGIN_ATTEMPT_MEMORY_MAX_ENTRIES = exports.LOGIN_ATTEMPT_REDIS_TIMEOUT_MS = exports.LOGIN_ATTEMPT_RESERVATION_MS = exports.LOGIN_ATTEMPT_MIN_SOURCES = exports.LoginAttemptScope = void 0;
...
const tracker_1 = require("../throttler/tracker");
/**
 ...
 */
var LoginAttemptScope;
(function (LoginAttemptScope) {
    LoginAttemptScope["PASSWORD"] = "password";
    LoginAttemptScope["MAGIC_CODE"] = "magic-code";
    LoginAttemptScope["TEAM_JOIN_CODE"] = "team-join-code";
})(LoginAttemptScope || (exports.LoginAttemptScope = LoginAttemptScope = {}));
/**
 ...
 */
exports.LOGIN_ATTEMPT_MIN_SOURCES = 2;
/**
 ...
 */
exports.LOGIN_ATTEMPT_RESERVATION_MS = 30_000;
/**
 ...
 */
exports.LOGIN_ATTEMPT_REDIS_TIMEOUT_MS = 250;
/**
 ...
 */
exports.LOGIN_ATTEMPT_MEMORY_MAX_ENTRIES = 50_000;
/**
 ...
 */
function shouldBlock(failures, distinctSources, input) {
    ...
    return failures >= input.maxFailures && (distinctSources >= exports.LOGIN_ATTEMPT_MIN_SOURCES || input.unresolvedSource);
}
/**
...
class MemoryLoginAttemptStore {
    ...
    entry(key, now) {
        ...
        if (!record) {
            if (this.records.size >= exports.LOGIN_ATTEMPT_MEMORY_MAX_ENTRIES) {
                this.evict(now);
            }
    ...
    async reserve(key, token, now) {
        const record = this.entry(key, now);
        record.inFlight.set(token, now + exports.LOGIN_ATTEMPT_RESERVATION_MS);
    ...
    async recordFailure(key, token, now, input) {
        const record = this.entry(key, now);
        record.inFlight.delete(token);
...
class RedisLoginAttemptStore {
    ...
    async reserve(key, token, now) {
        ...
        const results = await this.client
            .multi()
            .zRemRangeByScore(keys.inFlight, '-inf', now - exports.LOGIN_ATTEMPT_RESERVATION_MS)
            .zAdd(keys.inFlight, { score: now, value: token })
            .pExpire(keys.inFlight, exports.LOGIN_ATTEMPT_RESERVATION_MS)
            .zCard(keys.inFlight)
            .pTTL(keys.blocked)
            .exec();
        return {
    ...
    async recordFailure(key, token, _now, input) {
        const keys = this.keys(key);
...
let LoginAttemptService = LoginAttemptService_1 = class LoginAttemptService {
    constructor(redisClient) {
        ...
        this.memory = new MemoryLoginAttemptStore();
     ...
     */
    get maxFailures() {
        return config_1.environment.AUTH_MAX_FAILED_ATTEMPTS ?? 10;
    }
    /**
     ...
     */
    get lockoutMs() {
        return (config_1.environment.AUTH_LOCKOUT_SECONDS ?? 900) * 1000;
    }
    /**
    ...
    buildKey(scope, identifier) {
        const normalized = String(identifier ?? '')
            .trim()
            .toLowerCase();
        return `auth:fail:${scope}:${(0, node_crypto_1.createHash)('sha256').update(normalized).digest('hex')}`;
    }
    /**
     ...
     */
    currentSource() {
        let tracker = tracker_1.UNRESOLVED_THROTTLER_TRACKER;
        try {
            tracker = (0, tracker_1.resolveThrottlerTracker)(request_context_1.RequestContext.currentRequest(), {
        ...
        return {
            source: (0, node_crypto_1.createHash)('sha256').update(tracker).digest('hex').slice(0, 32),
            unresolvedSource: tracker === tracker_1.UNRESOLVED_THROTTLER_TRACKER
        };
    }
    /**
    ...
    async begin(scope, identifier) {
        ...
        let settled = false;
        const settle = (operation, task) => async () => {
            if (settled) {
                return;
            }
            settled = true;
        ...
        return {
            fail: settle('recordFailure', (store) => store.recordFailure(key, token, Date.now(), {
                ...source,
                maxFailures,
    ...
    reject(retryAfter) {
        try {
            request_context_1.RequestContext.currentRequest()?.res?.setHeader?.('Retry-After', String(retryAfter));
        }
        catch {
        ...
        throw new common_1.HttpException({
            ...
            message: `Too many failed attempts for this account. Try again in ${Math.ceil(retryAfter / 60)} minute(s).`,
            retryAfter
            ...
            try {
                return await Promise.race([
                    ...
                    new Promise((_, reject) => {
                        timer = setTimeout(() => reject(new Error(`Redis did not answer within ${exports.LOGIN_ATTEMPT_REDIS_TIMEOUT_MS}ms`)), exports.LOGIN_ATTEMPT_REDIS_TIMEOUT_MS);
                        timer.unref?.();
                    })
                ]);
            }
            catch (error) {
                this.logger.error(`Login attempt counter unavailable in Redis (${operation}), using in-process store: ${error?.message}`);
            }
            finally {
                clearTimeout(timer);
            }
...
exports.LoginAttemptService = LoginAttemptService = LoginAttemptService_1 = tslib_1.__decorate([
    (0, common_1.Injectable)(),
    tslib_1.__param(0, (0, common_1.Optional)()),
    tslib_1.__param(0, (0, common_1.Inject)(redis_module_1.EVER_REDIS_CLIENT)),
