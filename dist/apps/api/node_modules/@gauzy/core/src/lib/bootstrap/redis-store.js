 ...
 */
function skipSessionForHealthRoutes(middleware) {
    return (req, res, next) => req.path === '/api/health' || req.path.startsWith('/api/health/') ? next() : middleware(req, res, next);
}
/**
 ...
 */
async function configureRedisSession(app) {
    ...
    if (process.env.REDIS_ENABLED === 'true') {
        try {
            ...
            const redisStore = new connect_redis_1.default({
                client: redisClient,
                prefix: config_1.environment.production ? 'gauzyprodsess:' : 'gauzydevsess:'
            });
            app.use(skipSessionForHealthRoutes(expressSession({
                store: redisStore,
                secret: config_1.environment.EXPRESS_SESSION_SECRET,
