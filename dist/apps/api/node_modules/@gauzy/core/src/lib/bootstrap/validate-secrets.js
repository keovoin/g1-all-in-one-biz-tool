...
exports.validateApplicationSecrets = validateApplicationSecrets;
...
function validateApplicationSecrets() {
    ...
    const weak = KNOWN_DEFAULT_SECRETS.filter(({ key, value }) => {
        ...
        const current = String(env[key] ?? process.env[key] ?? '').trim();
        return !current || current === value;
    }).map(({ key }) => key);
    if (weak.length === 0) {
        return;
    }
    const guidance = 'Generate strong, unique values (e.g. `openssl rand -hex 64`) and provide them via environment ' +
    ...
    const isProduction = process.env.NODE_ENV === 'production' || config_1.environment.production === true;
    const isDemo = process.env.DEMO === 'true';
    if (isProduction && !isDemo) {
        if (process.env.ALLOW_INSECURE_JWT_SECRET === 'true') {
            // eslint-disable-next-line no-console
            console.error(chalk.red('Continuing despite insecure secrets because ALLOW_INSECURE_JWT_SECRET=true. This is STRONGLY discouraged.'));
            return;
        }
        throw new Error(`Refusing to start: ${weak.join(', ')} ${weak.length === 1 ? 'is' : 'are'} unset or use a ` +
 ...
 */
const KNOWN_DEFAULT_SEED_CREDENTIALS = [
 ...
 */
function resolveSeedPassword(key) {
    const credentials = (config_1.environment.demoCredentialConfig ?? {});
    const fromConfig = {
 ...
 */
function validateSeedCredentials(options = {}) {
    ...
    const problems = [];
    if (weak.length > 0) {
        problems.push(`${keys.join(', ')} ${keys.length === 1 ? 'is' : 'are'} unset or use the well-known default value ` +
    ...
    if (fixtures) {
        problems.push('This seed type also creates the DEFAULT_EVER_EMPLOYEES fixture accounts, whose password is ' +
    ...
    // eslint-disable-next-line no-console
    console.error(chalk.bgRed.whiteBright.bold(` INSECURE SEED CREDENTIALS: ${[...keys, ...(fixtures ? ['fixture accounts'] : [])].join(', ')} `));
    // eslint-disable-next-line no-console
    console.error(chalk.red(guidance));
    ...
    const isProduction = process.env.NODE_ENV === 'production' || config_1.environment.production === true;
    const isDemo = process.env.DEMO === 'true' || config_1.environment.demo === true;
    const isElectron = process.env.IS_ELECTRON === 'true' || config_1.environment.isElectron === true;
    if (isProduction && !isDemo && !isElectron) {
        if (process.env.ALLOW_INSECURE_SEED_CREDENTIALS === 'true') {
            // eslint-disable-next-line no-console
            console.error(chalk.red('Continuing despite insecure seed credentials because ALLOW_INSECURE_SEED_CREDENTIALS=true. ' +
                'This is STRONGLY discouraged — rotate these accounts immediately after the seed.'));
            return;
        }
        throw new Error(`Refusing to seed a production deployment: ${guidance} ` +
            '(To override temporarily, set ALLOW_INSECURE_SEED_CREDENTIALS=true — not recommended.)');
    }
}
//# sourceMappingURL=validate-secrets.js.map