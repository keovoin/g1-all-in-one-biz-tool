...
class TenantAwareCrudService extends crud_service_1.CrudService {
    constructor(typeOrmRepository, mikroOrmRepository) {
        ...
        this.skipEmployeeFilterKey = `skipEmployeeFilter:${this.constructor.name}:${++TenantAwareCrudService.skipEmployeeFilterSequence}`;
    }
    /**
     ...
     */
    getSkipEmployeeFilterDepth() {
        try {
            const context = context_1.RequestContext['clsService'];
     ...
     * Uses AsyncLocalStorage via RequestContext to avoid race conditions.
     */
    setSkipEmployeeFilterDepth(depth) {
        try {
            const context = context_1.RequestContext['clsService'];
    ...
    findConditionsWithEmployeeByUser() {
        ...
        const canChangeEmployee = context_1.RequestContext.hasPermission(contracts_1.PermissionsEnum.CHANGE_SELECTED_EMPLOYEE);
        // Restrict to current employee only
        if ((0, utils_1.isNotEmpty)(employeeId) && hasEmployeeColumn && !canChangeEmployee) {
            return {
     ...
     * ```
     */
    async withoutEmployeeFilter(callback) {
    ...
    */
     ...
     */
    async assertNotForeignRow(entity, tenantId) {
        const id = entity?.id;
        ...
        switch (this.ormType) {
            case utils_2.MultiORMEnum.MikroORM: {
                ...
                // reason.
                existing = await this.mikroOrmRepository.findOne({ id }, {
                    fields: ['id', 'tenantId'],
                    filters: false
                });
        ...
        if (existing && String(existingTenantId ?? '') !== String(tenantId)) {
            throw new common_1.ForbiddenException('The record belongs to another tenant');
        }
    }
    /**
     * Batch form of {@link assertNotForeignRow} for createMany()/saveMany() (one lookup for all ids).
     */
    async assertNotForeignRows(entities, tenantId) {
        ...
        switch (this.ormType) {
            case utils_2.MultiORMEnum.MikroORM:
                existing = await this.mikroOrmRepository.find({ id: { $in: ids } }, {
                    fields: ['id', 'tenantId'],
                    filters: false
                });
                break;
        ...
        if (existing.some((row) => String(row?.tenantId ?? '') !== String(tenantId))) {
            throw new common_1.ForbiddenException('One of the records belongs to another tenant');
        }
    }
    /**
    ...
    async create(entity) {
        ...
        const hasEmployeeColumn = this.typeOrmRepository.metadata?.hasColumnWithPropertyPath('employeeId');
        const hasPermission = context_1.RequestContext.hasPermission(contracts_1.PermissionsEnum.CHANGE_SELECTED_EMPLOYEE);
        return await super.create({
             ...
             */
            ...((0, utils_1.isNotEmpty)(employeeId) && !hasPermission && hasEmployeeColumn
                ? {
                    employee: { id: employeeId },
    ...
    async createMany(entities) {
        ...
        const hasEmployeeColumn = this.typeOrmRepository.metadata?.hasColumnWithPropertyPath('employeeId');
        const hasPermission = context_1.RequestContext.hasPermission(contracts_1.PermissionsEnum.CHANGE_SELECTED_EMPLOYEE);
        const shouldSetEmployee = (0, utils_1.isNotEmpty)(employeeId) && !hasPermission && hasEmployeeColumn;
     ...
     */
    async saveWithoutEnrichment(entity) {
     ...
     * @returns The array of saved entities.
     */
    async saveManyWithoutEnrichment(entities) {
    ...
    async delete(criteria, options) {
        try {
            ...
            (0, criteria_helper_1.assertCriteriaHasPredicate)(where, 'delete');
            const user = context_1.RequestContext.currentUser();
...
TenantAwareCrudService.skipEmployeeFilterSequence = 0;
