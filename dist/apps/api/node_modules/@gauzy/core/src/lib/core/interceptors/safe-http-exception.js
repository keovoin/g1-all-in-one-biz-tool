...
exports.toSafeHttpException = toSafeHttpException;
exports.sanitizeErrorBody = sanitizeErrorBody;
 ...
 *   successful response.
 */
function toSafeHttpException(error) {
    if (error instanceof common_1.BadRequestException) {
        ...
        if ((0, database_error_1.isDatabaseErrorPayload)(response)) {
            return new common_1.BadRequestException((0, database_error_1.describeDatabaseError)(response));
        }
        ...
        return new common_1.BadRequestException(sanitizeErrorBody(response));
    }
    if (error instanceof common_1.HttpException) {
        ...
        if ((0, database_error_1.isDatabaseErrorPayload)(response)) {
            return new common_1.HttpException((0, database_error_1.describeDatabaseError)(response), error.getStatus());
        }
        const safe = sanitizeErrorBody(response);
        // Nothing to scrub (the common case) → the ORIGINAL instance, subclass and all.
        if (JSON.stringify(safe) === JSON.stringify(response)) {
            return error;
        }
        return new common_1.HttpException(safe, error.getStatus());
    }
    const candidate = error?.status;
    const status = isHttpErrorStatus(candidate) ? candidate : common_1.HttpStatus.INTERNAL_SERVER_ERROR;
    const message = error?.message;
    return new common_1.HttpException(typeof message === 'string' && message ? message : 'Internal server error', status);
}
/**
 ...
 */
function sanitizeErrorBody(value, depth = 0) {
...
//# sourceMappingURL=safe-http-exception.js.map