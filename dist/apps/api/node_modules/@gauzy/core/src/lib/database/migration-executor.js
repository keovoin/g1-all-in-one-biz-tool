...
exports.resolveMigrationDirectory = resolveMigrationDirectory;
exports.createMigration = createMigration;
exports.initializeDatabaseConnection = initializeDatabaseConnection;
exports.shutdownDatabaseConnection = shutdownDatabaseConnection;
...
async function runDatabaseMigrations(pluginConfig) {
    ...
    try {
        ...
        if ((0, utils_1.isNotEmpty)(migrations)) {
...
async function generateMigration(pluginConfig, options) {
    ...
    try {
        ...
        if (upSqls.length) {
             ...
             */
            const fileContent = getTemplate(dataSource, options.name, timestamp, upSqls, downSqls.reverse());
 ...
 */
function resolveMigrationDirectory(options, config) {
    if (options.dir) {
        return options.dir;
    }
    try {
 ...
 */
async function initializeDatabaseConnection(config) {
 ...
 */
async function shutdownDatabaseConnection(dataSource) {
...
export class ${(0, StringUtils_1.camelCase)(name, true)}${timestamp} implements MigrationInterface {
    ...
    */
    public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${(0, utils_2.isDatabaseType)([config_1.DatabaseTypeEnum.postgres], connection.options)
        ? upSqls.join(`
        `)
        : [].join(`
        `)}
    ...
    */
    public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${(0, utils_2.isSqliteDB)(connection.options)
        ? upSqls.join(`
        `)
        : [].join(`
        `)}
    ...
    */
    ...
    public async mysqlUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${(0, utils_2.isDatabaseType)([config_1.DatabaseTypeEnum.mysql], connection.options) ? upSqls.join(``) : [].join(``)}
    }
    ...
    public async mysqlDownQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${(0, utils_2.isDatabaseType)([config_1.DatabaseTypeEnum.mysql], connection.options) ? downSqls.join(``) : [].join(``)}
    }
}
`;
}
