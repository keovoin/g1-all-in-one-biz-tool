...
exports.ManagedEmployeeService = void 0;
 ...
 */
let ManagedEmployeeService = class ManagedEmployeeService {
    constructor(typeOrmTeamEmployeeRepository, typeOrmProjectEmployeeRepository) {
        this.typeOrmTeamEmployeeRepository = typeOrmTeamEmployeeRepository;
        this.typeOrmProjectEmployeeRepository = typeOrmProjectEmployeeRepository;
    }
    /**
     ...
     */
    async filterAccessibleEmployeeIds(requestedEmployeeIds = [], teamIds = [], projectIds = [], onlyMe = false) {
        ...
        if ((0, utils_1.isNotEmpty)(teamIds) || (0, utils_1.isNotEmpty)(projectIds)) {
            ...
            if (isManager) {
                ...
                if ((0, utils_1.isNotEmpty)(requestedEmployeeIds)) {
     ...
     */
    async isManagerOfTeamsOrProjects(currentEmployeeId, teamIds = [], projectIds = []) {
        ...
        if ((0, utils_1.isNotEmpty)(projectIds)) {
            const isProjectManager = await this.typeOrmProjectEmployeeRepository.existsBy({
                employeeId: currentEmployeeId,
                organizationProjectId: (0, typeorm_1.In)(projectIds),
                isManager: true,
                isActive: true,
                isArchived: false,
                tenantId
     ...
     */
    async canManageEmployee(targetEmployeeId, organizationTeamId, organizationId) {
        const user = context_1.RequestContext.currentUser();
        const currentEmployeeId = user?.employeeId;
        // Case 1: User has global permission to change selected employee
        if (context_1.RequestContext.hasPermission(contracts_1.PermissionsEnum.CHANGE_SELECTED_EMPLOYEE)) {
     ...
     */
    async canViewEmployeeProfile(targetEmployeeId, organizationId, organizationTeamId) {
        const tenantId = context_1.RequestContext.currentTenantId();
        if (!tenantId) {
            return false;
        }
        ...
        const currentEmployeeId = context_1.RequestContext.currentUser()?.employeeId;
        if (currentEmployeeId === targetEmployeeId) {
            return true;
        }
        ...
        const memberships = await this.typeOrmTeamEmployeeRepository
            ...
            .getMany();
        const isExactActiveMembership = (membership) => {
            const team = membership.organizationTeam;
            return (membership.organizationTeamId === organizationTeamId &&
        ...
        const actorMemberships = memberships.filter((membership) => membership.employeeId === currentEmployeeId && isExactActiveMembership(membership));
        const targetMemberships = memberships.filter((membership) => membership.employeeId === targetEmployeeId && isExactActiveMembership(membership));
        if (!(0, utils_1.isNotEmpty)(actorMemberships) || !(0, utils_1.isNotEmpty)(targetMemberships)) {
            return false;
        }
        if (actorMemberships.some((membership) => membership.isManager === true)) {
            return true;
        }
        return (actorMemberships.some((membership) => membership.organizationTeam.shareProfileView === true) &&
     ...
     */
    async canManageEmployees(targetEmployeeIds, organizationTeamIds) {
        // No employees to check → return true
        if (!(0, utils_1.isNotEmpty)(targetEmployeeIds)) {
            return true;
        }
        // Check each employee
        for (const targetEmployeeId of targetEmployeeIds) {
            ...
            if ((0, utils_1.isNotEmpty)(organizationTeamIds)) {
     ...
     */
    async canManageEmployeeInAnyTeam(targetEmployeeId, organizationId) {
        ...
        if (!(0, utils_1.isNotEmpty)(managedTeams)) {
            return false;
        }
        ...
        const isTargetMember = await this.typeOrmTeamEmployeeRepository.existsBy({
            employeeId: targetEmployeeId,
            organizationTeamId: (0, typeorm_1.In)(managedTeamIds),
            isActive: true,
            isArchived: false,
            tenantId
        });
     ...
     */
    async getMembersOfTeamsAndProjects(teamIds = [], projectIds = []) {
        const tenantId = context_1.RequestContext.currentTenantId();
...
exports.ManagedEmployeeService = ManagedEmployeeService;
exports.ManagedEmployeeService = ManagedEmployeeService = tslib_1.__decorate([
...
//# sourceMappingURL=managed-employee.service.js.map