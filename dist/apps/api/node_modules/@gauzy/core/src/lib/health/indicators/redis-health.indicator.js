...
let RedisHealthIndicator = class RedisHealthIndicator {
    ...
    async startRedis() {
        ...
        if (!REDIS_URL && (!REDIS_HOST || !REDIS_PORT)) {
            console.warn('Redis is enabled but neither REDIS_URL nor REDIS_HOST/REDIS_PORT are configured. Redis Health Client is not enabled.');
            return false;
        }
        ...
        try {
            ...
            // Create Redis client
            this._client = (0, redis_1.createClient)(redisConnectionOptions).on('error', (err) => console.error('Redis Health Client Error', err));
