...
exports.insertBulkInvites = exports.createRandomEmployeeInviteSent = exports.createDefaultEmployeeInviteSent = void 0;
 ...
 */
const createDefaultEmployeeInviteSent = async (dataSource, tenant, organizations, superAdmins) => {
    const { id: tenantId } = tenant;
    const roles = await dataSource.manager.findBy(internal_1.Role, { tenantId });
    const totalInvites = organizations.flatMap((organization) => {
    ...
    return await (0, exports.insertBulkInvites)(dataSource, totalInvites);
};
 ...
 */
function createInvite({ superAdmins, organizationId, tenantId, roles }) {
    const now = new Date();
    const expireDateEnd = moment(now).add(30, 'days').toDate();
    const invite = new invite_entity_1.Invite();
    invite.email = (0, utils_1.getEmailWithPostfix)(faker_1.faker.internet.exampleEmail());
    invite.expireDate = faker_1.faker.date.between({ from: now, to: expireDateEnd });
    invite.invitedByUser = superAdmins.length
        ? faker_1.faker.helpers.arrayElement(superAdmins)
        : undefined; // Or fallback to a known user
    invite.organizationId = organizationId;
    invite.tenantId = tenantId;
    invite.role = faker_1.faker.helpers.arrayElement(roles);
    invite.status = faker_1.faker.helpers.arrayElement(Object.values(contracts_1.InviteStatusEnum));
 ...
 */
const insertBulkInvites = async (dataSource, invites, batchSize = 100) => {
    if (!invites.length) {
        console.warn('No invites to insert. Please check the input data and try again.');
        return [];
    }
    try {
        const repository = dataSource.getRepository(invite_entity_1.Invite);
...
exports.insertBulkInvites = insertBulkInvites;
