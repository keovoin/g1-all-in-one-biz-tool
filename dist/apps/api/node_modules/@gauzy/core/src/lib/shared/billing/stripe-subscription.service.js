...
exports.StripeSubscriptionService = exports.EntitlementResult = void 0;
...
(function (EntitlementResult) {
    ...
    EntitlementResult["ENTITLED"] = "entitled";
    /** Stripe answered, and this email has no entitling subscription. */
    EntitlementResult["NOT_ENTITLED"] = "not_entitled";
    /** Stripe could not be reached or errored. Caller decides; this is never a hard "no". */
    EntitlementResult["UNKNOWN"] = "unknown";
})(EntitlementResult || (exports.EntitlementResult = EntitlementResult = {}));
let StripeSubscriptionService = StripeSubscriptionService_1 = class StripeSubscriptionService {
    constructor() {
        this.logger = new common_1.Logger(StripeSubscriptionService_1.name);
        this.warned = new Set();
    }
    /**
    ...
    get secretKey() {
        ...
        if (process.env.DEMO === 'true') {
            this.warnOnce('demo', 'STRIPE_SECRET_KEY is set on a DEMO deployment. Billing is disabled: demo environments must never reach Stripe.');
            return undefined;
        }
        if (LIVE_KEY_PREFIX.test(key) && process.env.STRIPE_LIVE_MODE !== 'true') {
            this.warnOnce('live', 'A LIVE Stripe key is configured but STRIPE_LIVE_MODE is not "true". Billing is disabled rather than ' +
    ...
    async getEntitlement(email) {
        if (!this.secretKey)
            return EntitlementResult.ENTITLED; // billing off: nothing to check
        try {
            ...
            this.logger.error(`Could not determine Stripe entitlement for a registration attempt; allowing it through. ${describe(error)}`);
     ...
     */
    async findCustomerIdForEmail(email) {
        if (!this.secretKey)
            return null;
        try {
            ...
            this.logger.error(`Could not resolve a Stripe customer for a new tenant; it will need linking later. ${describe(error)}`);
            return null;
        }
    }
    /**
    ...
    async getCustomerEmail(customerId) {
        const key = this.secretKey;
        if (!key || !customerId?.trim())
            return null;
        try {
            const customer = await this.request(key, `/customers/${encodeURIComponent(customerId.trim())}`);
     ...
     *    alone.
     */
    async findEntitlingCustomerId(email) {
        const key = this.secretKey;
        if (!key)
            return null;
        ...
        for (const spelling of spellings) {
            for await (const customer of this.paginate(key, `/customers?email=${encodeURIComponent(spelling)}`, deadline)) {
    ...
    async hasEntitlingSubscription(key, customerId, deadline) {
        for await (const subscription of this.paginate(key, `/subscriptions?customer=${encodeURIComponent(customerId)}&status=all`, deadline)) {
     ...
     */
    async *paginate(key, path, deadline) {
        let startingAfter;
        ...
        for (;;) {
            ...
            const page = await this.request(key, `${path}${separator}limit=${PAGE_SIZE}${startingAfter ? `&starting_after=${startingAfter}` : ''}`);
...
exports.StripeSubscriptionService = StripeSubscriptionService = StripeSubscriptionService_1 = tslib_1.__decorate([
