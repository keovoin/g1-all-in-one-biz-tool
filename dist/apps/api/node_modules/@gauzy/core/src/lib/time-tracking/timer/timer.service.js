...
let TimerService = class TimerService {
    constructor(typeOrmTimeLogRepository, mikroOrmTimeLogRepository, typeOrmEmployeeRepository, mikroOrmEmployeeRepository, _employeeService, _commandBus) {
    ...
    async getTimerStatus(request) {
        ...
        (0, sensitive_relations_helper_1.assertSensitiveRelationsAllowed)(this.typeOrmTimeLogRepository.metadata, request.relations);
        ...
        if (!!permission && (0, utils_1.isNotEmpty)(request.employeeId)) {
        ...
        /** */
        const { start, end } = (0, utils_2.getDateRangeFormat)(moment.utc(todayStart || moment().startOf('day')), moment.utc(todayEnd || moment().endOf('day')));
        let logs = [];
        let lastLog;
        ...
        switch (this.ormType) {
            case utils_2.MultiORMEnum.MikroORM:
                 ...
                 */
                const parseQueryParams = (0, utils_2.parseTypeORMFindToMikroOrm)((0, timer_helper_1.buildLogQueryParameters)(queryParams));
                ...
                logs = items.map((entity) => (0, utils_2.wrapSerialize)(entity));
                /**
                ...
                const parseMikroOrmOptions = (0, utils_2.parseTypeORMFindToMikroOrm)(lastLogQueryParamsMikroOrm);
                // Get today's last log (running or completed)
            ...
            case utils_2.MultiORMEnum.TypeORM:
                ...
                const completedLogQueryParams = (0, timer_helper_1.buildLogQueryParameters)(queryParams);
                ...
                lastLog = await this.typeOrmTimeLogRepository.findOne({
                    ...
                    relations: (0, utils_2.parseFindOptionsRelations)(lastLogQueryParamsTypeOrm.relations)
                });
                break;
            default:
    ...
    async startTimer(request) {
        console.log(`-------------Start Timer Request (${moment.utc(request.startedAt).toDate()})-------------`, JSON.stringify(request));
        // Destructure the necessary parameters from the request
        const { source, logType, projectId, taskId, organizationContactId, organizationTeamId, description, isBillable, version } = request;
        ...
        const timeLog = await this._commandBus.execute(new commands_1.TimeLogCreateCommand({
            organizationId,
            tenantId,
            employeeId,
            startedAt,
    ...
    async stopTimer(request) {
        console.log(`-------------Stop Timer Request (${moment.utc(request.stoppedAt).toDate()})-------------`, JSON.stringify(request));
        // Validate the date range and check if the timer is running
        (0, utils_2.validateDateRange)(request.startedAt, request.stoppedAt);
        ...
        if (!lastLog) {
            ...
            throw new common_1.NotAcceptableException(`No running log found. Can't stop timer because it was already stopped.`);
        }
        ...
        if (stoppedAt < lastLog.startedAt) {
            console.log(chalk.yellow(`stoppedAt (${stoppedAt}) is less than startedAt (${lastLog.startedAt}), skipping stoppedAt update.`));
        }
        // Construct the update payload, conditionally excluding stoppedAt if it shouldn't be updated
        const partialTimeLog = {
            isRunning: false,
        ...
        lastLog = await this._commandBus.execute(new commands_1.TimeLogUpdateCommand(partialTimeLog, lastLog.id, request.manualTimeSlot));
        // Update the employee's tracking status to reflect they are now tracking time
        await this._employeeService.update(employeeId, {
     ...
     */
    async handleConflictingTimeLogs(lastLog, tenantId, organizationId, forceDelete = false) {
        try {
            // Validate the date range and check if the timer is running
            (0, utils_2.validateDateRange)(lastLog.startedAt, lastLog.stoppedAt);
            // Retrieve conflicting time logs
            const conflicts = await this._commandBus.execute(new commands_1.IGetConflictTimeLogCommand({
                ignoreId: lastLog.id,
            ...
            if (conflicts?.length) {
                ...
                for await (const timeLog of conflicts) {
                    ...
                    for await (const timeSlot of timeSlots) {
                        await this._commandBus.execute(new commands_1.DeleteTimeSpanCommand(times, timeLog, timeSlot, forceDelete));
                    }
                }
            }
        }
     ...
     */
    async calculateStoppedAt(request, lastLog) {
        ...
        if (request.source === contracts_1.TimeLogSourceEnum.DESKTOP) {
            // Retrieve the most recent time slot from the last log
            const lastTimeSlot = lastLog.timeSlots?.sort((a, b) => moment(b.startedAt).diff(a.startedAt))[0];
            // Example:
     ...
     */
    async calculateStoppedAt2(request, lastLog) {
        // Retrieve stoppedAt date or default to the current date if not provided
        let stoppedAt = moment.utc(request.stoppedAt ?? moment.utc()).toDate();
     ...
     */
    async stopPreviousRunningTimers(employeeId, organizationId, tenantId) {
        try {
            ...
            if (logs.length > 0) {
                ...
                await this._commandBus.execute(new commands_1.ScheduleTimeLogEntriesCommand(employeeId, organizationId, tenantId));
            }
     ...
     */
    async findEmployee() {
        const userId = context_1.RequestContext.currentUserId(); // Get the current user ID
        const tenantId = context_1.RequestContext.currentTenantId(); // Get the current tenant ID
        ...
        if (!employee) {
            throw new common_1.NotFoundException('Employee record not found. Please verify your details and try again.');
        }
        return employee;
    }
    /**
     ...
     */
    async getRunningLogs(fetchAll = false, includeTimeSlots = false) {
        const tenantId = context_1.RequestContext.currentTenantId(); // Retrieve the tenant ID from the current context
        ...
        const whereClause = {
            ...
            organizationId,
            isRunning: true,
            stoppedAt: (0, typeorm_1.Not)((0, typeorm_1.IsNull)()) // Logs should have a non-null `stoppedAt`
        };
        switch (this.ormType) {
            case utils_2.MultiORMEnum.MikroORM: {
                const findOptions = (0, utils_2.parseTypeORMFindToMikroOrm)({
                    where: whereClause,
                ...
                if (fetchAll) {
                    ...
                    return items.map((entity) => (0, utils_2.wrapSerialize)(entity));
                }
                else {
                    const item = await this.mikroOrmTimeLogRepository.findOne(findOptions.where, findOptions.mikroOptions);
                    return item ? (0, utils_2.wrapSerialize)(item) : null;
                }
            }
            case utils_2.MultiORMEnum.TypeORM:
            default:
     ...
     */
    async getLastRunningLog(includeTimeSlots = false) {
    ...
    async getTimerWorkedStatus(request) {
        ...
        if (context_1.RequestContext.hasAnyPermission(permissions)) {
            ...
            employeeIds = [context_1.RequestContext.currentEmployeeId()];
        }
        let lastLogs = [];
        switch (this.ormType) {
            case utils_2.MultiORMEnum.MikroORM:
                ...
                const { mikroOptions } = (0, utils_2.parseTypeORMFindToMikroOrm)({
                    ...(request.relations ? { relations: request.relations } : {})
                });
                // Get last logs group by employees (running or completed);
                lastLogs = (await this.mikroOrmTimeLogRepository.find({ id: { $in: timeLogIds } }, mikroOptions)).map((item) => (0, utils_2.wrapSerialize)(item));
                break;
            case utils_2.MultiORMEnum.TypeORM:
                /**
                ...
                query.setFindOptions({
                    ...(request['relations'] ? { relations: (0, utils_2.parseFindOptionsRelations)(request['relations']) } : {})
                });
                query.where({
                    ...
                    isActive: true,
                    isArchived: false,
                    ...((0, utils_1.isNotEmpty)(source) ? { source } : {}),
                    ...((0, utils_1.isNotEmpty)(organizationTeamId) ? { organizationTeamId } : {})
                });
...
exports.TimerService = TimerService = tslib_1.__decorate([
