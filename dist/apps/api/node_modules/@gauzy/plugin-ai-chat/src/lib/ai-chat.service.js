...
exports.AiChatService = exports.MAX_AUDIO_BYTES = void 0;
...
const speech_provider_error_1 = require("./speech/speech-provider-error");
const system_prompt_1 = require("./system-prompt");
const gauzy_api_client_1 = require("./tools/gauzy-api-client");
const gauzy_tools_1 = require("./tools/gauzy-tools");
const client_tools_1 = require("./tools/client-tools");
const mcp_tools_1 = require("./tools/mcp-tools");
const data_parts_1 = require("./tools/data-parts");
const tool_registry_1 = require("./tools/tool-registry");
const ai_provider_credential_service_1 = require("./credentials/ai-provider-credential.service");
const ai_chat_conversation_service_1 = require("./conversations/ai-chat-conversation.service");
 ...
 */
exports.MAX_AUDIO_BYTES = 25 * 1024 * 1024;
...
let AiChatService = AiChatService_1 = class AiChatService {
    constructor(credentialService, conversationService) {
    ...
    async streamChat(args) {
        if (!Array.isArray(args.messages) || args.messages.length === 0) {
            throw new common_1.BadRequestException('messages must be a non-empty array of UI messages.');
        }
        const ai = await (0, esm_loader_1.loadAiSdk)();
        ...
        const requestDefaults = {
            ...
            tenantId: core_1.RequestContext.currentTenantId() ?? undefined,
            employeeId: core_1.RequestContext.currentEmployeeId() ?? undefined
        };
        const apiClient = new gauzy_api_client_1.GauzyApiClient(args.authorizationHeader, {
        ...
        const dataParts = (0, data_parts_1.createDeferredDataPartWriter)();
        ...
        const [gauzyTools, clientTools, mcp, registryContribution] = await Promise.all([
            (0, gauzy_tools_1.buildGauzyTools)(apiClient, requestDefaults),
            (0, client_tools_1.buildClientTools)(),
            (0, mcp_tools_1.createMcpTools)(args.authorizationHeader),
            tool_registry_1.AiChatToolRegistry.resolveAll({
        ...
        const instructions = (0, system_prompt_1.buildSystemPrompt)({
            userName: user?.name,
        ...
        const registryApproval = (registryContribution.requireApproval ?? []).filter((name) => tools[name] === registryContribution.tools[name]);
        const approvalRequired = [
            ...gauzy_tools_1.GAUZY_TOOLS_REQUIRING_APPROVAL,
            ...client_tools_1.CLIENT_TOOLS_REQUIRING_APPROVAL,
            ...mcpToolNames,
            ...registryApproval
        ];
        let result;
        try {
            result = ai.streamText({
                ...
                messages: await ai
                    ...
                    throw new common_1.BadRequestException(`Invalid chat messages payload: ${error instanceof Error ? error.message : error}`);
                }),
                tools,
                ...
                onError: (event) => {
                    ...
                    this.logger.error(`streamText error [provider=${providerId} model=${modelId} credential=${credentialSource}]: ` +
        ...
        const maskError = (error) => {
            if (!(0, rate_limit_1.isRateLimitError)(error))
                return 'An error occurred.';
            const retryAfterSeconds = (0, rate_limit_1.rateLimitRetryAfter)(error);
            return (0, rate_limit_1.buildRateLimitEnvelope)({
                code: rate_limit_1.RATE_LIMIT_CODE,
                providerId,
                credentialSource,
        ...
        const modelStream = ai.toUIMessageStream({
            ...
            onEnd: async ({ messages }) => {
                ...
                dataParts.release();
                if (!args.conversationId || !persistFor.userId || !persistFor.tenantId)
                    return;
                try {
                    ...
                    this.logger.warn(`Failed to persist conversation ${args.conversationId}: ${error instanceof Error ? error.message : error}`);
                }
            }
        });
     ...
     */
    async getConfig() {
        const definitions = provider_registry_1.AiProviderRegistry.list();
        const providers = [];
    ...
    async resolveModel(requestedProviderId, requestedModelId) {
        const definitions = provider_registry_1.AiProviderRegistry.list();
        if (!definitions.length) {
            throw new common_1.ServiceUnavailableException('No AI providers are registered.');
        }
        let definition;
        if (requestedProviderId) {
            definition = provider_registry_1.AiProviderRegistry.get(requestedProviderId);
            if (!definition) {
                throw new common_1.BadRequestException(`Unknown AI provider '${requestedProviderId}'.`);
            }
            ...
            if (definition.chatCapable === false) {
                throw new common_1.ServiceUnavailableException(`AI provider '${definition.label}' cannot serve chat yet — select another provider.`);
            }
        }
        else {
            ...
            const configured = (await Promise.all(selectable.map(async (candidate) => ((await this.resolveCredentials(candidate)) ? candidate : null)))).filter((candidate) => candidate !== null);
            ...
            definition = defaults ? provider_registry_1.AiProviderRegistry.get(defaults.defaultProvider) : configured[0];
        }
        if (!definition) {
            throw new common_1.ServiceUnavailableException('AI chat is not configured: no provider has credentials (tenant settings or server environment).');
        }
        const credentials = await this.resolveCredentials(definition);
        if (!credentials) {
            throw new common_1.ServiceUnavailableException(`AI provider '${definition.id}' has no usable credentials.`);
        }
        ...
        if (platformModels) {
            ...
            if (!platformModels.length) {
                throw new common_1.ServiceUnavailableException(`AI provider '${definition.id}' has no free models available right now. ` +
            ...
            if (requestedModelId && !allowed.has(requestedModelId)) {
                ...
                throw new common_1.BadRequestException(`Model '${requestedModelId}' is not available on the free tier. ` +
     ...
     */
    async resolveCredentials(definition) {
    ...
    async listProviderModels(providerId) {
        const definition = provider_registry_1.AiProviderRegistry.get(providerId);
        if (!definition) {
            throw new common_1.BadRequestException(`Unknown AI provider '${providerId}'.`);
        }
        ...
        try {
            ...
            this.logger.warn(`[ai-chat] Model catalogue fetch failed for '${providerId}'; serving the curated list: ` +
     ...
     */
    async transcribe(audio, mimeType, options) {
        if (!audio?.length) {
            throw new common_1.BadRequestException('No audio was uploaded.');
        }
        ...
        if (audio.length > exports.MAX_AUDIO_BYTES) {
            throw new common_1.BadRequestException(`Recording is too large (${Math.round(audio.length / 1024 / 1024)}MB). The limit is ${exports.MAX_AUDIO_BYTES / 1024 / 1024}MB.`);
        }
        const capable = provider_registry_1.AiProviderRegistry.list()
            .filter((definition) => typeof definition.transcribe === 'function')
            .sort((a, b) => (a.order ?? Number.MAX_SAFE_INTEGER) - (b.order ?? Number.MAX_SAFE_INTEGER));
        if (!capable.length) {
            throw this.speechUnavailable(contracts_1.AiSpeechErrorCode.NOT_CONFIGURED, 'No AI provider on this server can transcribe speech — install a speech-capable provider plugin.');
        }
        ...
        for (const definition of ordered) {
            ...
            try {
                ...
                // problem by accident.
                if ((0, speech_provider_error_1.isSpeechProviderError)(error) && error.kind === 'key-rejected') {
                    keyRejected = true;
                }
        ...
        if (!failures.length) {
            ...
            throw this.speechUnavailable(contracts_1.AiSpeechErrorCode.NOT_CONFIGURED, `Dictation needs a voice provider. Add one (${capable
                .slice(0, 4)
        ...
        throw this.speechUnavailable(keyRejected ? contracts_1.AiSpeechErrorCode.KEY_REJECTED : contracts_1.AiSpeechErrorCode.FAILED, `${detail}.${keyRejected ? ' Update the key on the AI Providers settings page.' : ''}`, attempted);
    }
    /**
     ...
     */
    speechUnavailable(code, message, attemptedProviders) {
        const body = {
            message,
            code,
            settingsPath: contracts_1.AI_CHAT_SETTINGS_PATH,
            ...(attemptedProviders?.length ? { attemptedProviders } : {})
        };
        return new common_1.ServiceUnavailableException(body);
    }
    /**
     ...
     */
    async resolveVoiceDefault(capableIds) {
     ...
     */
    async resolvePlatformModels(definition, credentials) {
        if (credentials.source !== 'platform' || !definition.listPlatformModels)
            return null;
        try {
            ...
            this.logger.error(`[ai-chat] Could not list platform models for '${definition.id}'; refusing the platform tier.`, error instanceof Error ? error.stack : String(error));
            return [];
        }
    }
    async getTenantCredential(providerId) {
        const tenantId = core_1.RequestContext.currentTenantId();
        if (!tenantId)
            return null;
        try {
    ...
    async resolveDefaultProvider(configuredIds) {
        const tenantId = core_1.RequestContext.currentTenantId();
        if (tenantId) {
            try {
        ...
        if (!first)
            return null;
        const definition = provider_registry_1.AiProviderRegistry.get(first);
...
exports.AiChatService = AiChatService = AiChatService_1 = tslib_1.__decorate([
    ...
    tslib_1.__metadata("design:paramtypes", [ai_provider_credential_service_1.AiProviderCredentialService,
        ai_chat_conversation_service_1.AiChatConversationService])
], AiChatService);
