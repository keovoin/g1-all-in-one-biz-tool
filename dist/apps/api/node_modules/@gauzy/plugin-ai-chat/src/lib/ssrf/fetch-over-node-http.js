...
exports.fetchOverNodeHttp = fetchOverNodeHttp;
 ...
 */
async function fetchOverNodeHttp(input, init, options) {
...
function send(url, method, headers, body, signal, lookup) {
    return new Promise((resolve, reject) => {
        ...
        if (!body) {
            ...
            (0, stream_1.pipeline)(stream_1.Readable.fromWeb(body.stream), req, (error) => {
                if (error)
                    req.destroy(error);
            });
        }
...
function outgoingHeaders(headers, url, method, body) {
    const outgoing = {};
...
function contentDecoders(header) {
    ...
    for (let i = codings.length - 1; i >= 0; i--) {
        ...
        if (coding === 'gzip' || coding === 'x-gzip')
            decoders.push(zlib.createGunzip(ZLIB_OPTIONS));
        else if (coding === 'deflate')
            decoders.push(zlib.createInflate(ZLIB_OPTIONS));
        else if (coding === 'br')
            decoders.push(zlib.createBrotliDecompress(BROTLI_OPTIONS));
...
function toResponse(incoming, method, url, redirected, signal) {
    const status = incoming.statusCode ?? 0;
    ...
    if (method !== 'HEAD' && !NULL_BODY_STATUSES.has(status)) {
        ...
        for (const decoder of decoders) {
            ...
            body = (0, stream_1.pipeline)(body, decoder, () => undefined);
        }
    }
    else {
        incoming.resume();
    ...
    let response;
    try {
        response = new Response(body ? stream_1.Readable.toWeb(body) : null, {
            status,
...
//# sourceMappingURL=fetch-over-node-http.js.map