 ...
 */
export var RangePeriod;
(function (RangePeriod) {
    RangePeriod["DAY"] = "DAY";
    RangePeriod["WEEK"] = "WEEK";
    RangePeriod["PERIOD"] = "PERIOD";
})(RangePeriod || (RangePeriod = {}));
/** Identity of the six counter widgets (array index in the default order = `position`). */
export var Widgets;
(function (Widgets) {
    Widgets[Widgets["MEMBERS_WORKED"] = 0] = "MEMBERS_WORKED";
    Widgets[Widgets["PROJECTS_WORKED"] = 1] = "PROJECTS_WORKED";
    Widgets[Widgets["TODAY_ACTIVITY"] = 2] = "TODAY_ACTIVITY";
    Widgets[Widgets["WORKED_TODAY"] = 3] = "WORKED_TODAY";
    Widgets[Widgets["WORKED_THIS_WEEK"] = 4] = "WORKED_THIS_WEEK";
    Widgets[Widgets["WEEKLY_ACTIVITY"] = 5] = "WEEKLY_ACTIVITY";
})(Widgets || (Widgets = {}));
/** Identity of the six windows — same enum as the Angular `Windows`. */
export var Windows;
(function (Windows) {
    Windows[Windows["RECENT_ACTIVITIES"] = 0] = "RECENT_ACTIVITIES";
    Windows[Windows["MANUAL_TIMES"] = 1] = "MANUAL_TIMES";
    Windows[Windows["TASKS"] = 2] = "TASKS";
    Windows[Windows["PROJECTS"] = 3] = "PROJECTS";
    Windows[Windows["APPS_URLS"] = 4] = "APPS_URLS";
    Windows[Windows["MEMBERS"] = 5] = "MEMBERS";
})(Windows || (Windows = {}));
 ...
 * range exists).
 */
export function resolveRangePeriod(range) {
    if (!range)
        return undefined;
 ...
 */
export function isCurrentWeek(range, now = moment()) {
    if (!range)
        return false;
    return (moment(range.startDate).format('YYYY-MM-DD') === now.clone().startOf('week').format('YYYY-MM-DD') &&
 ...
 */
export function periodCapacity(range, organization, employeesCount) {
    if (!range?.startDate || !range?.endDate || employeesCount === undefined || employeesCount === null)
        return undefined;
    ...
    const workDay = startWork?.isValid() && endWork?.isValid() && endWork.isAfter(startWork) ? endWork.diff(startWork) / 1000 : 86400;
 ...
 */
export function widgetTitleKeys(period, currentWeek) {
    const keys = [
 ...
 * @param currentWeek Whether the range is the current week.
 */
export function titleMapper(position, isWidget, period, currentWeek) {
 ...
 * @param period Detected period.
 */
export function emptyMessageKey(baseKey, period) {
