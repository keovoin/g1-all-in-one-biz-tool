...
import { DocumentKindEnum, BaseEntityEnum } from '@gauzy/contracts';
...
import * as i7 from "@ngneat/effects-ng";
 ...
 */
export class DocsRowActionsService {
    constructor(translateService, documentsService, exportService, treeStore, dialogService, toastrService, actions, router, favoriteStore, genericFavoriteService, store) {
        ...
        this._favoriteIds$ = new BehaviorSubject(new Set());
        this.favoriteIds$ = this._favoriteIds$.asObservable();
    ...
    // ─── Create / rename / move ──────────────────────────────────
    async createChild(target, kind) {
        const created = await firstValueFrom(this.dialogService.open(CreateDialogComponent, { context: { kind, parentId: target.id } }).onClose);
        if (!created)
            return false;
    ...
    async rename(target) {
        const renamed = await firstValueFrom(this.dialogService.open(CreateDialogComponent, {
            context: {
                kind: target.kind,
    ...
    async move(target) {
        const moved = await firstValueFrom(this.dialogService.open(MoveDialogComponent, { context: { documentIds: [target.id] } }).onClose);
        if (!moved)
            return false;
    ...
    async toggleFavorite(target) {
        ...
        const favorites = await this.genericFavoriteService.loadFavorites(BaseEntityEnum.Document, organization, employeeId);
        await this.genericFavoriteService.toggleFavorite(BaseEntityEnum.Document, String(target.id), organization, employeeId, favorites);
        return true;
    }
    /** Deep link to the row: a PAGE opens its editor route, anything else the detail panel. */
    deepLink(target) {
        const path = target.kind === DocumentKindEnum.PAGE
    ...
    // ─── Mutations ───────────────────────────────────────────────
    async setKnowledge(target, include) {
     ...
     */
    async setArchived(target, archive) {
     ...
     */
    async remove(target) {
        const result = await firstValueFrom(this.dialogService.open(DocsDeleteDialogComponent, { context: { target } }).onClose);
        if (!result?.strategy)
            return false;
    ...
    documentIdOfLink(link) {
        const value = String(link ?? '');
        if (!value.startsWith(DOCS_PAGE_LINK))
            return null;
        return value.split('?id=')[1]?.split('&')[0] || null;
    }
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocsRowActionsService, deps: [{ token: i1.TranslateService }, { token: i2.DocumentsService }, { token: i3.DocsExportService }, { token: i4.DocumentTreeStore }, { token: i5.NbDialogService }, { token: i6.ToastrService }, { token: i7.Actions }, { token: i8.Router }, { token: i6.FavoriteStoreService }, { token: i6.GenericFavoriteService }, { token: i6.Store }], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocsRowActionsService }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocsRowActionsService, decorators: [{
            type: Injectable
        }], ctorParameters: () => [{ type: i1.TranslateService }, { type: i2.DocumentsService }, { type: i3.DocsExportService }, { type: i4.DocumentTreeStore }, { type: i5.NbDialogService }, { type: i6.ToastrService }, { type: i7.Actions }, { type: i8.Router }, { type: i6.FavoriteStoreService }, { type: i6.GenericFavoriteService }, { type: i6.Store }] });
//# sourceMappingURL=docs-row-actions.service.js.map