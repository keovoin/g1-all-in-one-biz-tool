...
export const blockCommentsPluginKey = new PluginKey('gzBlockComments');
 ...
 */
export function enclosingBlockId(editor) {
 ...
 */
export function setCommentedBlocks(editor, blockIds) {
...
function createMarker(blockId, options) {
    ...
    addProseMirrorPlugins() {
        ...
        return [
            new Plugin({
                ...
                state: {
                    init: () => [],
                    apply: (transaction, value) => transaction.getMeta(blockCommentsPluginKey) ?? value
                },
                props: {
                    decorations(state) {
                        ...
                        state.doc.descendants((node, pos) => {
                            ...
                            if (typeof blockId !== 'string' || !wanted.has(blockId))
                                return true;
                            decorations.push(Decoration.node(pos, pos + node.nodeSize, { class: 'gz-block-commented' }));
                            // `side: -1` + a stable key: the widget renders at the block's own
                            // flow position and is only rebuilt when the anchor set changes.
                            decorations.push(Decoration.widget(pos, () => createMarker(blockId, options), {
                                side: -1,
...
//# sourceMappingURL=block-comments.plugin.js.map