...
export class DocumentAutosaveService {
    constructor() {
        ...
        this._conflict$ = new BehaviorSubject(null);
        this.conflict$ = this._conflict$.asObservable();
     ...
     */
    init(documentId, updatedAt, payloadProvider) {
        this.clearTimers();
        this.session += 1;
    ...
    markDirty() {
        if (this.frozen || !this.documentId)
            return;
        this.dirty = true;
    ...
    async flush(options = {}) {
        if (!this.documentId || this.frozen)
            return false;
        if (!this.dirty && !options.forceSnapshot)
            return true;
        ...
        if (payload.contentBinary)
            body.contentBinary = payload.contentBinary;
        try {
            const saved = await firstValueFrom(this.documentsService.updateContent(documentId, body));
    ...
    schedule(callback, delay) {
        return this.zone.runOutsideAngular(() => setTimeout(() => this.zone.run(callback), delay));
    }
    clearTimers() {
        if (this.debounceTimer)
            clearTimeout(this.debounceTimer);
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocumentAutosaveService, deps: [], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocumentAutosaveService }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocumentAutosaveService, decorators: [{
            type: Injectable
        }] });
//# sourceMappingURL=document-autosave.service.js.map