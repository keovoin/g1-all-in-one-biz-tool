...
exports.sampleMarkdown = sampleMarkdown;
exports.buildClassificationPrompt = buildClassificationPrompt;
exports.parseClassificationOutput = parseClassificationOutput;
...
function buildClassificationPrompt(input) {
    const system = 'You classify business documents for an organization on the Ever Gauzy platform.\n' +
    ...
    const user = `Category catalog (slug: description), pick 1-3 that fit best:\n${input.catalogLines}\n\n` +
        `File name: ${input.originalFilename}\n\n` +
        `${(0, untrusted_content_1.fenceDocumentContent)(input.sampledMarkdown)}\n\n` +
 ...
 */
function parseClassificationOutput(raw, validSlugs) {
    if (!raw) {
        return null;
    }
    ...
    return { categories: categories, suggestedTags: suggestedTags, summary, language, confidence };
}
//# sourceMappingURL=classification.prompt.js.map