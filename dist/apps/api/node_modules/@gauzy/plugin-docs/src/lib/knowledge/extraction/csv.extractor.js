...
exports.CsvExtractor = exports.CSV_MAX_ROWS = void 0;
exports.detectDelimiter = detectDelimiter;
exports.parseCsv = parseCsv;
exports.csvRowsToMarkdown = csvRowsToMarkdown;
 ...
 */
function consumeQuotedField(text, start, accumulator) {
    let i = start;
    while (i < text.length) {
        const ch = text[i];
        if (ch !== '"') {
 ...
 */
function csvRowsToMarkdown(rows, sheetName, totalNote) {
    if (rows.length === 0) {
        return `## Sheet: ${sheetName}\n\n_Empty file._`;
    }
    const width = Math.max(...rows.map((row) => row.length));
    const pad = (row) => {
        const cells = row.map((cell) => (0, extractor_interface_1.escapeTableCell)(cell));
 ...
 */
let CsvExtractor = class CsvExtractor {
    /**
     * @inheritdoc
     */
    supports(mime) {
    ...
    async extract(buffer, ctx) {
        const text = (0, extractor_interface_1.normalizeMarkdown)(buffer.toString('utf8'));
        const delimiter = detectDelimiter(text);
        const { rows, capped } = parseCsv(text, delimiter, exports.CSV_MAX_ROWS);
        const stem = (ctx.filename ?? 'data').replace(/\.[A-Za-z0-9]+$/, '') || 'data';
        const kept = rows.slice(0, exports.CSV_MAX_ROWS);
        const note = capped
            ? `_Truncated: only the first ${exports.CSV_MAX_ROWS} rows are shown._`
            : undefined;
        const rendered = csvRowsToMarkdown(kept, stem, note);
        const { markdown, truncated } = (0, extractor_interface_1.capMarkdown)(rendered, ctx.maxChars);
        return {
            markdown,
            metadata: {
                pageCount: 1,
                ...
                warnings: capped ? [`Row cap applied (${exports.CSV_MAX_ROWS})`] : undefined
            }
        };
    }
};
exports.CsvExtractor = CsvExtractor;
exports.CsvExtractor = CsvExtractor = tslib_1.__decorate([
