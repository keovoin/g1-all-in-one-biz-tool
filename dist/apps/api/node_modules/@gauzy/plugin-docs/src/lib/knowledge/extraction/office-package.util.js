...
exports.OFFICE_PACKAGE_MAX_TOTAL_BYTES = exports.OFFICE_PACKAGE_MAX_ENTRY_BYTES = void 0;
exports.openOfficePackage = openOfficePackage;
...
exports.OFFICE_PACKAGE_MAX_ENTRY_BYTES = 64 * 1024 * 1024;
/** Largest total inflated size one package may yield across all the entries an extractor reads. */
exports.OFFICE_PACKAGE_MAX_TOTAL_BYTES = 192 * 1024 * 1024;
 ...
 */
function openOfficePackage(buffer) {
    if (!buffer || buffer.length < 22) {
        throw new errors_1.DocsPermanentError('The file could not be read — it is not a valid office package.');
    }
    const eocdOffset = findEndOfCentralDirectory(buffer);
    if (eocdOffset < 0) {
        throw new errors_1.DocsPermanentError('The file could not be read — it may be corrupt, password-protected, or not an office document.');
    }
    ...
    if (centralOffset === ZIP64_SENTINEL || centralSize === ZIP64_SENTINEL || entryCount === 0xffff) {
        ...
        throw new errors_1.DocsPermanentError('ZIP64 office packages are not supported — re-save the file and try again.');
    }
    if (centralOffset + centralSize > buffer.length) {
        throw new errors_1.DocsPermanentError('The file could not be read — its archive index is truncated.');
    }
    ...
    const readEntry = (entry) => {
        if (entry.uncompressedSize > exports.OFFICE_PACKAGE_MAX_ENTRY_BYTES) {
            throw new errors_1.DocsPermanentError('The office package contains an implausibly large part and was refused.');
        }
        const data = extractEntryBytes(buffer, entry);
        inflatedTotal += data.length;
        if (inflatedTotal > exports.OFFICE_PACKAGE_MAX_TOTAL_BYTES) {
            throw new errors_1.DocsPermanentError('The office package expands to an implausible size and was refused.');
        }
        return data;
    };
    return {
...
function readCentralDirectory(buffer, offset, entryCount) {
    ...
    for (let i = 0; i < entryCount; i++) {
        if (cursor + 46 > buffer.length || buffer.readUInt32LE(cursor) !== SIGNATURE_CENTRAL) {
            throw new errors_1.DocsPermanentError('The file could not be read — its archive index is malformed.');
        }
...
function extractEntryBytes(buffer, entry) {
    ...
    if (headerOffset + 30 > buffer.length || buffer.readUInt32LE(headerOffset) !== SIGNATURE_LOCAL) {
        throw new errors_1.DocsPermanentError(`The office package part '${entry.name}' could not be located.`);
    }
    ...
    if (end > buffer.length) {
        throw new errors_1.DocsPermanentError(`The office package part '${entry.name}' is truncated.`);
    }
    ...
    if (entry.method === METHOD_DEFLATE) {
        try {
            return (0, zlib_1.inflateRawSync)(payload, { maxOutputLength: exports.OFFICE_PACKAGE_MAX_ENTRY_BYTES });
        }
        catch (error) {
            throw new errors_1.DocsPermanentError(`The office package part '${entry.name}' could not be decompressed.`, error);
        }
    }
    throw new errors_1.DocsPermanentError(`The office package uses an unsupported compression method (${entry.method}) — re-save the file and try again.`);
}
//# sourceMappingURL=office-package.util.js.map