...
const file_sniffer_1 = require("./file-sniffer");
/** Sources a client may claim on the upload endpoint — everything else is reserved. */
const UPLOAD_SOURCE_ALLOWLIST = [contracts_1.DocumentSourceEnum.UPLOAD, contracts_1.DocumentSourceEnum.CHAT, contracts_1.DocumentSourceEnum.EDITOR];
/**
 ...
 */
const QUOTA_WARN_ONLY_SOURCES = [contracts_1.DocumentSourceEnum.CHAT, contracts_1.DocumentSourceEnum.EMAIL];
/**
...
let DocumentUploadService = DocumentUploadService_1 = class DocumentUploadService {
    constructor(typeOrmDocumentRepository, documentService, documentSettingsService, documentQuotaService, processingService) {
     ...
     */
    async uploadDocuments(input, files) {
        ...
        const source = input.source ?? contracts_1.DocumentSourceEnum.UPLOAD;
     ...
     */
    async assertBatchAccepted(input, files, provider, source) {
        if (!files || files.length === 0) {
            throw new common_1.BadRequestException({ message: 'No files were uploaded', code: docs_constants_1.DOCS_FILE_TYPE_REJECTED });
        }
        ...
        if (!UPLOAD_SOURCE_ALLOWLIST.includes(source)) {
            ...
            throw new common_1.BadRequestException({
                ...
                code: docs_constants_1.DOCS_SOURCE_RESERVED
            });
        }
     ...
     */
    async resolveBatchContext(input, seed) {
     ...
     */
    async processFile(file, fileName, context) {
        try {
            ...
            if (file.size > context.maxFileSize) {
                return {
                    oversize: true,
                    rejection: {
                        code: docs_constants_1.DOCS_FILE_TOO_LARGE,
            ...
            const buffer = (await context.provider.getFile(file.key));
            const sniff = (0, file_sniffer_1.sniffFile)(buffer, fileName, file.mimetype);
            if (!sniff.ok) {
            ...
            this.logger.error(`Upload failed for ${fileName.slice(0, 40)}: ${error.message}`);
            return { rejection: { code: docs_constants_1.DOCS_FILE_TYPE_REJECTED, message: 'The file could not be processed' } };
        }
    }
    /**
    ...
    rejectOverQuota(file, context) {
        ...
        if (context.quotaWarnOnly) {
            this.logger.warn(`Organization ${organizationId} is over its documents storage quota ` +
        ...
        return {
            rejection: {
                code: docs_constants_1.DOCS_QUOTA_EXCEEDED,
                message: `The organization storage quota of ${quotaState.quotaBytes} bytes would be ` +
     ...
     */
    async findDuplicate(sha256, context) {
     ...
     */
    async storeAcceptedFile(file, fileName, sniff, sha256, context) {
        const { input } = context;
        ...
        const document = await this.documentService.create({
            ...
            knowledgeStatus: context.importToKnowledge
                ...
                : contracts_1.DocumentKnowledgeStatusEnum.NONE,
     ...
     */
    throwBatchRejected(rejected, allOversize) {
        if (allOversize) {
        ...
        throw new common_1.BadRequestException({
            ...
            code: docs_constants_1.DOCS_FILE_TYPE_REJECTED,
            rejected
        });
    }
    /**
     ...
     */
    async replaceFile(id, file, input = {}) {
        const provider = new core_1.FileStorage().getProvider();
        if (!file?.key) {
            throw new common_1.BadRequestException({
        ...
        try {
            ...
            const buffer = (await provider.getFile(file.key));
            const sniff = (0, file_sniffer_1.sniffFile)(buffer, fileName, file.mimetype);
            if (!sniff.ok) {
                throw new common_1.BadRequestException({ message: sniff.message, code: sniff.code });
            }
            await this.persistReplacedFile(document, file, fileName, sniff, (0, crypto_1.createHash)('sha256').update(buffer).digest('hex'), provider);
        }
        catch (error) {
            await this.cleanupOne(provider, file);
            throw error;
        ...
        if (previousStorageKey && previousStorageKey !== file.key) {
            await this.cleanupOne(new core_1.FileStorage().getProvider(previousProvider), { key: previousStorageKey });
        }
        ...
        await this.processingService.enqueueExtract(document, 'replace', { keepExtractedText: false, classify: input.classifyWithAi }, { jobId: `docs:extract:${document.id}:${Date.now()}` });
        return document;
    }
    /**
     ...
     */
    async persistReplacedFile(document, file, fileName, sniff, sha256, provider) {
        const existingMetadata = (document.metadata && typeof document.metadata === 'object' ? document.metadata : {});
        const metadata = {
            ...existingMetadata,
            upload: {
                declaredMimeType: file.mimetype ?? null,
    ...
    async getDownloadUrl(id) {
        ...
        if (!url) {
            throw new common_1.NotFoundException(`Document ${id} has no retrievable file`);
        }
        return { url };
    }
    /**
     ...
     */
    async cleanupOne(provider, file) {
        ...
        try {
            ...
            this.logger.error(`Failed to delete rejected upload blob: ${error.message}`);
        }
    }
    /**
     * Deletes every stored blob of a batch (batch-level rejections).
     */
    async cleanupAll(provider, files) {
        for (const file of files) {
            await this.cleanupOne(provider, file);
        }
    }
...
exports.DocumentUploadService = DocumentUploadService = DocumentUploadService_1 = tslib_1.__decorate([
