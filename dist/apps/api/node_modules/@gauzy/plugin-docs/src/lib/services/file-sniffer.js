...
exports.SNIFF_WINDOW_BYTES = void 0;
exports.extractExtension = extractExtension;
exports.isMarkupContent = isMarkupContent;
exports.isProbablyUtf8Text = isProbablyUtf8Text;
exports.zipHasEntry = zipHasEntry;
exports.sniffBinarySignature = sniffBinarySignature;
exports.sniffFile = sniffFile;
exports.canonicalExtension = canonicalExtension;
/** Number of leading bytes inspected by the signature checks. */
exports.SNIFF_WINDOW_BYTES = 4096;
 ...
 */
function isMarkupContent(buffer) {
    if (!buffer || buffer.length === 0) {
        return false;
    }
    // UTF-32 / UTF-16 BOMs (a raster image never begins with these).
    if (startsWith(buffer, [0xff, 0xfe, 0x00, 0x00]) || // UTF-32 LE
 ...
 */
function hasWideEncodingBom(window) {
    return (startsWith(window, [0xff, 0xfe]) ||
 ...
 */
function utf8ContinuationCount(byte) {
    if ((byte & 0xe0) === 0xc0) {
        return 1;
    }
    if ((byte & 0xf0) === 0xe0) {
 ...
 */
function readUtf8Sequence(window, index) {
    ...
    if (index + extra >= window.length && window.length === exports.SNIFF_WINDOW_BYTES) {
...
function isProbablyUtf8Text(buffer) {
    ...
    const window = buffer.subarray(0, exports.SNIFF_WINDOW_BYTES);
...
function zipHasEntry(buffer, predicate) {
    let offset = 0;
    let guard = 0;
 ...
 */
function readOdfMimetype(buffer) {
 ...
 */
function sniffFile(buffer, filename, declaredMime) {
 ...
 * @returns Accept/reject with the canonical type or a stable error code.
 */
function sniffTextPayload(buffer, extension, declaredMime) {
 ...
 */
function acceptWithConsistency(type, extension, declaredMime) {
    ...
    if (extension) {
        ...
        if (expected && expected !== type.mimeType) {
            return reject('DOCS_TYPE_MISMATCH', `File extension .${extension} does not match its content (${type.mimeType})`);
        }
    }
    ...
    if (declaredMime && declaredMime !== 'application/octet-stream') {
        ...
        if (normalized && normalized !== type.mimeType) {
            return reject('DOCS_TYPE_MISMATCH', `Declared type ${declaredMime} does not match the file content (${type.mimeType})`);
        }
    }
    return { ok: true, type };
}
...
//# sourceMappingURL=file-sniffer.js.map