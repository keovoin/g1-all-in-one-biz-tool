...
exports.DOCS_SCHEMA_MARKS = exports.DOCS_SCHEMA_NODES = void 0;
exports.validateTiptapDocument = validateTiptapDocument;
exports.stripTransientAttributes = stripTransientAttributes;
exports.collectDocumentMentionIds = collectDocumentMentionIds;
 ...
 */
const COMMON_NODE_ATTRIBUTES = ['blockId'];
/**
...
const TRANSIENT_ATTRIBUTES = ['uploadId'];
...
function reject(path, reason) {
    throw new common_1.BadRequestException({
        ...
        code: docs_constants_1.DOCS_CONTENT_SCHEMA_INVALID,
        path
    });
}
/**
 ...
 */
function isAllowedUrl(raw, schemes, allowRelative) {
 ...
 */
function validateAttributes(attrs, allowed, path, common) {
...
function validateMarks(marks, path) {
    ...
    marks.forEach((mark, index) => {
        ...
        if (typeof type !== 'string' || !Object.prototype.hasOwnProperty.call(exports.DOCS_SCHEMA_MARKS, type)) {
            reject(markPath, `unknown mark type "${String(type)}"`);
        }
        validateAttributes(mark['attrs'], exports.DOCS_SCHEMA_MARKS[type], markPath, []);
 ...
 */
function validateNode(node, path, depth, counter) {
    ...
    if (typeof type !== 'string' || !Object.prototype.hasOwnProperty.call(exports.DOCS_SCHEMA_NODES, type)) {
        reject(path, `unknown node type "${String(type)}"`);
    }
    validateAttributes(node['attrs'], exports.DOCS_SCHEMA_NODES[type], path, COMMON_NODE_ATTRIBUTES);
    ...
    content.forEach((child, index) => validateNode(child, `${path}.content[${index}]`, depth + 1, counter));
}
/**
 ...
 */
function validateTiptapDocument(value) {
 ...
 */
function stripTransientAttributes(value) {
    if (Array.isArray(value)) {
        return value.map((item) => stripTransientAttributes(item));
    }
    if (!isPlainObject(value)) {
        return value;
    }
 ...
 */
function collectDocumentMentionIds(value) {
    const ids = new Set();
    const walk = (node) => {
...
//# sourceMappingURL=tiptap-schema.validator.js.map