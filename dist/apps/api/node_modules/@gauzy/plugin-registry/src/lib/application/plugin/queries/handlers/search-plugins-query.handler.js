...
exports.SearchPluginsQueryHandler = void 0;
const tslib_1 = require("tslib");
const core_1 = require("@gauzy/core");
const cqrs_1 = require("@nestjs/cqrs");
const domain_1 = require("../../../../domain");
const shared_1 = require("../../../../shared");
const search_plugins_query_1 = require("../search-plugins.query");
// Get the type of the Object-Relational Mapping (ORM) used in the application.
const ormType = (0, core_1.getORMType)();
/**
 * Query handler for searching and filtering plugins with advanced criteria
 */
let SearchPluginsQueryHandler = class SearchPluginsQueryHandler {
    ...
    async searchAndFilter(filters) {
        const { search, name, description, type, status, isActive, categoryId, author, license, uploadedById, uploadedAfter, uploadedBefore, downloadedAfter, downloadedBefore, minDownloads, maxDownloads, version, tags, hasInstallations, isVerified, skip = 1, take = 10, sortBy: rawSortBy = 'createdAt', sortDirection: rawSortDirection = 'DESC', relations = [] } = filters;
        ...
        const sortBy = shared_1.PLUGIN_SORTABLE_FIELDS.includes(rawSortBy)
            ? rawSortBy
            : 'createdAt';
        const normalizedSortDirection = String(rawSortDirection).toUpperCase();
        const sortDirection = shared_1.PLUGIN_SORT_DIRECTIONS.includes(normalizedSortDirection)
        ...
        switch (ormType) {
            case core_1.MultiORMEnum.MikroORM: {
                ...
                const knex = this.pluginService.mikroOrmPluginRepository.getKnex();
                let qb = knex('plugins');
                ...
                if (name)
                    qb = qb.andWhere('plugins.name', name);
                if (description)
                    qb = qb.andWhere('plugins.description', 'ILIKE', `%${description}%`);
                if (type)
                    qb = qb.andWhere('plugins.type', type);
                if (status)
                    qb = qb.andWhere('plugins.status', status);
                if (isActive !== undefined)
                    qb = qb.andWhere('plugins.isActive', isActive);
                if (categoryId)
                    qb = qb.andWhere('plugins.categoryId', categoryId);
                if (author)
                    qb = qb.andWhere('plugins.author', 'ILIKE', `%${author}%`);
                if (license)
                    qb = qb.andWhere('plugins.license', 'ILIKE', `%${license}%`);
                if (uploadedById)
                    qb = qb.andWhere('plugins.uploadedById', uploadedById);
                if (uploadedAfter)
                    qb = qb.andWhere('plugins.uploadedAt', '>=', uploadedAfter);
                if (uploadedBefore)
                    qb = qb.andWhere('plugins.uploadedAt', '<=', uploadedBefore);
                if (downloadedAfter)
                    qb = qb.andWhere('plugins.lastDownloadedAt', '>=', downloadedAfter);
                if (downloadedBefore)
                    qb = qb.andWhere('plugins.lastDownloadedAt', '<=', downloadedBefore);
                // Version filter
                if (version) {
                ...
                if (sortBy === 'downloadCount') {
                    ...
                    qb = qb.orderByRaw('COALESCE("downloadStats"."totalDownloads", 0) ' + sortDirection.toUpperCase());
                }
                else {
                    qb = qb.orderBy(`plugins.${sortBy}`, sortDirection.toLowerCase());
                }
            ...
            default: {
                ...
                if (search) {
                    queryBuilder.andWhere('(plugin.name ILIKE :search OR plugin.description ILIKE :search OR plugin.author ILIKE :search)', { search: `%${search}%` });
                }
                // Exact name match
                if (name) {
                ...
                if (sortBy === 'downloadCount') {
                    ...
                    queryBuilder.orderBy('COALESCE(downloadStats.totalDownloads, 0)', sortDirection.toUpperCase());
                }
                else {
                    // Standard sorting for other fields
                    queryBuilder.orderBy(`plugin.${sortBy}`, sortDirection.toUpperCase());
                }
                // Pagination
...
exports.SearchPluginsQueryHandler = SearchPluginsQueryHandler;
exports.SearchPluginsQueryHandler = SearchPluginsQueryHandler = tslib_1.__decorate([
    (0, cqrs_1.QueryHandler)(search_plugins_query_1.SearchPluginsQuery),
    tslib_1.__metadata("design:paramtypes", [domain_1.PluginService])
], SearchPluginsQueryHandler);
//# sourceMappingURL=search-plugins-query.handler.js.map