...
import { PLUGIN_PERMISSION_CHECKER, PLUGIN_FEATURE_CHECKER } from '../plugin-ui.helper';
...
export class PageExtensionRegistryService {
    ...
    _permissionChecker = inject(PLUGIN_PERMISSION_CHECKER, { optional: true }) ?? null;
    _featureChecker = inject(PLUGIN_FEATURE_CHECKER, { optional: true }) ?? null;
    // Extension storage with BehaviorSubject for reactivity
    _extensions$ = new BehaviorSubject(new Map());
    // Slot definitions for dynamic slots
    _slots$ = new BehaviorSubject(new Map());
    // Plugin to extension mapping for cleanup
    _pluginToExtensions = new Map();
    // Plugin to slot mapping for cleanup
    _pluginToSlots = new Map();
    // ─────────────────────────────────────────────────────────────────────────
     ...
     * @param options Optional pluginId for lifecycle cleanup
     */
    registerAll(extensions, options) {
     ...
     */
    _registerTo(extension, extensionsMap, options) {
        ...
        if (slot?.maxExtensions && slot.maxExtensions > 0 && list.length > slot.maxExtensions) {
            console.warn(`Slot '${slotId}' has reached max extensions (${slot.maxExtensions}). ` +
     ...
     */
    getExtensions$(slotId) {
        return this._extensions$.pipe(map((extensions) => extensions.get(slotId)), distinctUntilChanged(), map((extensions) => extensions ?? []));
    }
    /**
     ...
     */
    async getVisibleExtensions(slotId, context) {
     ...
     */
    getVisibleExtensions$(slotId, context) {
        return this.getExtensions$(slotId).pipe(map((extensions) => {
     ...
     */
    async _isExtensionVisible(ext, context) {
        // Check hidden flag
        if (ext.hidden) {
            return false;
        }
     ...
     */
    _isExtensionVisibleSync(ext, _context) {
        if (ext.hidden) {
            return false;
        }
        ...
        if (ext.permissions?.length && this._permissionChecker) {
            if (!this._permissionChecker.hasAllPermissions(...ext.permissions))
                return false;
        }
        if (ext.permissionsAny?.length && this._permissionChecker) {
            if (!this._permissionChecker.hasAnyPermission(...ext.permissionsAny))
                return false;
        }
        // Check feature flag synchronously (IPluginFeatureChecker is sync)
        if (ext.featureKey && this._featureChecker) {
            if (!this._featureChecker.isFeatureEnabled(ext.featureKey))
                return false;
        }
        return true;
    }
     ...
     */
    async _checkPermissions(permissions, _context, mode) {
     ...
     * Falls back to allowing access when no checker is provided.
     */
    async _checkFeature(featureKey, _context) {
    ...
    static ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: PageExtensionRegistryService, deps: [], target: i0.ɵɵFactoryTarget.Injectable });
    static ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: PageExtensionRegistryService, providedIn: 'root' });
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: PageExtensionRegistryService, decorators: [{
            type: Injectable,
            args: [{ providedIn: 'root' }]
        }] });
//# sourceMappingURL=page-extension-registry.service.js.map