...
exports.SchedulerJobRegistryService = void 0;
const tslib_1 = require("tslib");
const common_1 = require("@nestjs/common");
const scheduler_constants_1 = require("../constants/scheduler.constants");
let SchedulerJobRegistryService = class SchedulerJobRegistryService {
    ...
    resolveJobOptions(metadata, jobId) {
        ...
        const queueName = queueNameInput && queueNameInput.length > 0
            ? queueNameInput
         ...
         */
        const unschedulableReason = enabled && queueName && !this.moduleOptions.enableQueueing
        ...
        const retryDelayMs = this.toNonNegativeInteger(metadata.retryDelayMs ?? this.moduleOptions.defaultJobOptions.retryDelayMs, 'retryDelayMs', jobId);
        const maxRandomDelayMs = this.toNonNegativeInteger(metadata.maxRandomDelayMs ?? this.moduleOptions.defaultJobOptions.maxRandomDelayMs, 'maxRandomDelayMs', jobId);
        const timeoutMs = metadata.timeoutMs !== undefined
    ...
    toNonNegativeInteger(value, fieldName, jobId) {
        if (!Number.isFinite(value) || value < 0) {
            throw new Error(`Job "${jobId}" has invalid "${fieldName}" value "${value}".`);
        }
        return Math.floor(value);
    }
    toPositiveInteger(value, fieldName, jobId) {
        if (!Number.isFinite(value) || value <= 0) {
            throw new Error(`Job "${jobId}" has invalid "${fieldName}" value "${value}".`);
        }
        return Math.floor(value);
...
exports.SchedulerJobRegistryService = SchedulerJobRegistryService;
exports.SchedulerJobRegistryService = SchedulerJobRegistryService = tslib_1.__decorate([
...
//# sourceMappingURL=scheduler-job-registry.service.js.map