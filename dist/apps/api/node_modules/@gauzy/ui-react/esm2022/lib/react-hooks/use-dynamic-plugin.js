...
export function useDynamicPlugin() {
    ...
    const loader = useMemo(() => {
        try {
            return injector.get(DynamicPluginLoaderService);
        }
        catch {
            return null;
        }
    ...
    const fallback$ = useMemo(() => of([]), []);
    const loadedIds = useObservable(loader?.loadedPluginIds$ ?? fallback$, loader?.loadedPluginIds ?? []);
    const load = useCallback(async (definition) => {
        if (!loader) {
            const result = {
                success: false,
    ...
    const unload = useCallback(async (pluginId) => {
        if (!loader) {
            const result = {
                success: false,
                pluginId,
    ...
    const reload = useCallback(async (definition) => {
        if (!loader) {
            const result = {
                success: false,
                pluginId: definition.id,
    ...
    const isLoaded = useCallback((pluginId) => {
...
//# sourceMappingURL=use-dynamic-plugin.js.map