...
exports.assertCriteriaHasPredicate = assertCriteriaHasPredicate;
...
function hasPredicate(value) {
    ...
    if (value instanceof typeorm_1.FindOperator || value instanceof Date) {
        return true;
    }
    ...
    if (typeof value === 'object') {
        return Object.values(value).some((nested) => hasPredicate(nested));
    }
    return true;
}
/**
 ...
 */
function assertCriteriaHasPredicate(criteria, method) {
    if (criteria === undefined || criteria === null || criteria === '') {
        throw new common_1.BadRequestException(`Empty criteria(s) are not allowed for the ${method} method.`);
    }
    if (typeof criteria === 'object' && !(criteria instanceof Date) && !(criteria instanceof typeorm_1.FindOperator)) {
        if (!hasPredicate(criteria)) {
            throw new common_1.BadRequestException(`Empty criteria(s) are not allowed for the ${method} method.`);
        }
    }
}
//# sourceMappingURL=criteria.helper.js.map