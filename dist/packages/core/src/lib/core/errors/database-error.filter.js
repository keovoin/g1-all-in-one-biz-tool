 ...
 */
let DatabaseErrorFilter = DatabaseErrorFilter_1 = class DatabaseErrorFilter extends core_1.BaseExceptionFilter {
    constructor() {
        super(...arguments);
        this.logger = new common_1.Logger(DatabaseErrorFilter_1.name);
    }
    /**
     ...
     */
        ...
        this.logger.error(`Database error on ${request?.method ?? ''} ${request?.url ?? ''}`, this.describeForLog(payload) ?? exception.stack);
        ...
        const isDriverErrorItself = (0, database_error_1.isDatabaseErrorPayload)(payload);
        const body = !isDriverErrorItself && payload && typeof payload === 'object'
            ? {
                ...this.withoutNestedDatabaseFields(payload),
                message: safeMessage
            }
            : {
                statusCode: status,
        ...
        super.catch(new common_1.HttpException(body, status), host);
    }
    /**
     ...
     */
    withoutNestedDatabaseFields(value, seen = new WeakSet(), depth = 0) {
        ...
        if (depth > DatabaseErrorFilter_1.MAX_SANITIZE_DEPTH) {
            return undefined;
        }
        if (seen.has(value)) {
            return undefined;
        ...
        if (Array.isArray(value)) {
            return value
                .filter((item) => !(0, database_error_1.isDatabaseErrorPayload)(item))
                .map((item) => this.withoutNestedDatabaseFields(item, seen, depth + 1));
        }
        const cleaned = {};
        for (const [key, item] of Object.entries(value)) {
            if ((0, database_error_1.isDatabaseErrorPayload)(item)) {
                continue;
            }
     ...
     * @param depth - Current depth.
     */
    redactForLog(value, seen = new WeakSet(), depth = 0) {
        if ((0, database_error_1.isDatabaseErrorPayload)(value)) {
            return (0, database_error_1.redactDatabaseError)(value);
        }
        if (typeof value === 'string') {
            // Driver text arriving as a plain message still names constraints and, on MySQL, values.
            return (0, database_error_1.safeMessageForDatabaseText)(value) ?? value;
        }
        if (!value || typeof value !== 'object') {
            return value;
        }
     ...
     * @param payload - The exception's response payload.
     */
    resolveSafeMessage(payload) {
        // Shape 1: the driver error object itself (`query` / `parameters` / `driverError`).
        if ((0, database_error_1.isDatabaseErrorPayload)(payload)) {
            return (0, database_error_1.describeDatabaseError)(payload);
        }
        // Shape 2: a plain string, from the sites that throw `error.message`.
        if (typeof payload === 'string') {
            return (0, database_error_1.safeMessageForDatabaseText)(payload);
        }
        // Shape 3: a normal Nest body whose `message` is the driver's text.
        if (payload && typeof payload === 'object') {
            const { message } = payload;
            if ((0, database_error_1.isDatabaseErrorPayload)(message)) {
                return (0, database_error_1.describeDatabaseError)(message);
            }
            return (0, database_error_1.safeMessageForDatabaseText)(message);
        }
        return undefined;
    }
};
...
DatabaseErrorFilter.MAX_SANITIZE_DEPTH = 6;
exports.DatabaseErrorFilter = DatabaseErrorFilter = DatabaseErrorFilter_1 = tslib_1.__decorate([
