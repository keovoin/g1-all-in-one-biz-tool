 ...
 */
function copyAssets(filename, config, destDir) {
    if (!destDir) {
    ...
    try {
        ...
        const sourceDir = isElectron ? path.join(config_1.environment.gauzySeedPath, destDir) : customSeedDir;
        // File path
...
async function cleanAssets(config, destDir) {
    ...
    const dir = config_1.environment.isElectron
        ? path.resolve(config_1.environment.gauzyUserPath, 'public', destDir) // Electron-specific path
