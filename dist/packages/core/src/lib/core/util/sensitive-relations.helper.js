...
exports.SENSITIVE_RELATION_SELF_KEY = void 0;
exports.isValidPermission = isValidPermission;
exports.getRequiredPermissionForRelation = getRequiredPermissionForRelation;
exports.assertSensitiveRelationsAllowed = assertSensitiveRelationsAllowed;
const common_1 = require("@nestjs/common");
const contracts_1 = require("@gauzy/contracts");
const context_1 = require("../context");
const utils_1 = require("../utils");
const organization_sensitive_relations_config_1 = require("./organization-sensitive-relations.config");
/**
 ...
 */
exports.SENSITIVE_RELATION_SELF_KEY = '_self';
/**
...
[ORGANIZATION_RELATION_PROPERTY]: organizationSubTree, ...topLevelOrganizationRelations } = organization_sensitive_relations_config_1.ORGANIZATION_SENSITIVE_RELATIONS;
 ...
 */
const SENSITIVE_ORGANIZATION_RELATION_NAMES = new Set(Object.keys(ORGANIZATION_RELATION_PERMISSIONS).filter((key) => key !== exports.SENSITIVE_RELATION_SELF_KEY));
/**
...
function isValidPermission(value) {
    return typeof value === 'string' && Object.values(contracts_1.PermissionsEnum).includes(value);
}
/**
 ...
 */
function getRequiredPermissionForRelation(config, relationPath) {
    ...
    for (const part of pathParts) {
        ...
        if (typeof value === 'object' && value !== null) {
            const self = value[exports.SENSITIVE_RELATION_SELF_KEY];
 ...
 */
function resolveSegmentRule(config, segment) {
    const rule = config[segment];
    if (rule && typeof rule === 'object') {
        const self = rule[exports.SENSITIVE_RELATION_SELF_KEY];
        return {
 ...
 */
function isOrganizationEntity(metadata) {
 ...
 */
function assertSensitiveRelationsAllowed(metadata, relations) {
    ...
    if (!context_1.RequestContext.currentRequestContext()) {
        return;
    }
    const paths = (0, utils_1.normalizeRelationsToPaths)(relations);
    if (paths.length === 0) {
        return;
    }
    // Cheap pre-filter: unless a requested segment is a name the table declares, no walk is needed.
    const touchesSensitiveName = paths.some((path) => path.split('.').some((segment) => SENSITIVE_ORGANIZATION_RELATION_NAMES.has(segment)));
    ...
    const hasPermission = (permission) => {
        ...
        const verdict = context_1.RequestContext.hasPermission(permission);
        verdicts.set(permission, verdict);
        return verdict;
    };
    for (const path of paths) {
        let entityMetadata = metadata;
        let config = isOrganizationEntity(entityMetadata)
        ...
        for (const segment of path.split('.')) {
            if (config) {
                ...
                if (permission && !hasPermission(permission)) {
                    throw new common_1.ForbiddenException(`Access to the sensitive relation '${path}' is forbidden. Required permission: '${permission}'.`);
                }
                config = next;
            }
            if (!entityMetadata) {
...
//# sourceMappingURL=sensitive-relations.helper.js.map