...
exports.UniqueTenantStripeCustomer1790000009000 = void 0;
 ...
 */
class UniqueTenantStripeCustomer1790000009000 {
    constructor() {
        this.name = 'UniqueTenantStripeCustomer1790000009000';
    }
    async up(queryRunner) {
        ...
        if (duplicates.length) {
            console.log(chalk.red(`${this.name}: ${duplicates.length} tenant(s) shared a Stripe customer with another tenant. ` +
        ...
        switch (type) {
            case config_1.DatabaseTypeEnum.postgres:
                ...
                await queryRunner.query(`CREATE UNIQUE INDEX "IDX_tenant_stripe_customer_id" ON "tenant" ("stripeCustomerId")`);
                break;
    ...
    async findDuplicates(queryRunner, type) {
        ...
        const minId = type === config_1.DatabaseTypeEnum.postgres ? 'MIN("id"::text)::uuid' : 'MIN("id")';
        const q = type === config_1.DatabaseTypeEnum.mysql
        ...
        const rows = await queryRunner.query(q);
        return (rows ?? []).map((r) => r.id);
    }
    /** Sets those same rows back to NULL. Same predicate, so the two cannot drift apart. */
    clearDuplicatesSql(type) {
        const minId = type === config_1.DatabaseTypeEnum.postgres ? 'MIN("id"::text)::uuid' : 'MIN("id")';
        return type === config_1.DatabaseTypeEnum.mysql
...
exports.UniqueTenantStripeCustomer1790000009000 = UniqueTenantStripeCustomer1790000009000;
//# sourceMappingURL=1790000009000-UniqueTenantStripeCustomer.js.map