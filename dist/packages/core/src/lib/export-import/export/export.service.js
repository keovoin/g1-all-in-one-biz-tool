...
let ExportService = ExportService_1 = class ExportService {
    constructor(repositoriesServices) {
     ...
     */
    async getAsCsv(job, item, where, organizationId) {
        const conditions = {};
         ...
         */
        if ((0, utils_1.isNotEmpty)(item.substitute) && (0, utils_1.isNotEmpty)(conditions['where'])) {
            const { substitute: { originalField = 'tenantId', substituteField = 'id' } } = item;
        ...
        if (defaultItemsCount > 0) {
            defaultItems.forEach((el) => {
                ...
                el['tenantId'] = context_1.RequestContext.currentTenantId();
            });
        }
        if (count > 0) {
    ...
    redactRows(repository, rows) {
        ...
        if (!(0, utils_1.isFunction)(entity)) {
            throw new TypeError(`Refusing to export "${repository.metadata.tableName}": its entity class could not be resolved, so redaction marks cannot be read`);
        }
        return rows.map((row) => (0, export_redact_decorator_1.redactForExport)(entity, row, columns));
    }
    /**
     ...
     */
    async csvWriter(job, filename, items) {
        if (!(0, utils_1.isNotEmpty)(items)) {
            return;
        }
     ...
     */
    async csvTemplateWriter(job, filename, columns) {
        if (!(0, utils_1.isNotEmpty)(columns)) {
            return;
        }
     ...
     */
    async downloadToUser(job, res) {
        return new Promise((resolve, reject) => {
            res.download(job.archivePath, job.archiveName, (error) => {
                if (!error) {
                    return resolve();
                }
     ...
     * @param organizationId - Organization the global default rows are stamped with.
     */
    async exportSpecificTables(job, names, organizationId) {
        const repositories = await this.getRepositories();
        for await (const item of repositories) {
     ...
     */
    async exportRelationalTables(job, entity, where) {
     ...
     */
    assertJunctionRowsOnly(junction, tableName, rows) {
        const allowed = new Set((junction?.columns ?? [])
            .filter((column) => column.relationMetadata || column.referencedColumn)
            .map((column) => column.databaseName));
        for (const row of rows) {
            const unexpected = Object.keys(row).filter((key) => !allowed.has(key));
            if (unexpected.length > 0) {
                throw new TypeError(`Refusing to export junction table "${tableName}": unexpected column(s) ${unexpected.join(', ')} cannot be redacted`);
            }
        }
    }
    /**
     ...
     * @param job - The job being exported.
     */
    async exportSpecificTablesSchema(job) {
        const repositories = await this.getRepositories();
        for await (const item of repositories) {
     ...
     */
    async exportRelationalTablesSchema(job, entity) {
        const { repository, relations } = entity;
        for await (const item of repository.metadata.manyToManyRelations) {
            const relation = relations.find((relation) => relation.joinTableName === item.joinTableName);
            if (relation) {
...
exports.ExportService = ExportService = ExportService_1 = tslib_1.__decorate([
    (0, common_1.Injectable)(),
    tslib_1.__metadata("design:paramtypes", [repositories_service_1.RepositoriesService])
], ExportService);
