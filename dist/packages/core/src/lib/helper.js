...
const path_util_1 = require("./core/util/path-util");
/**
 ...
 */
async function resolveServeStaticPath(config) {
    // Default public directory for assets
    const publicDir = (0, path_util_1.getApiPublicPath)();
    console.log(chalk.green(`✔ Server Static Config -> publicDir: ${publicDir}`));
    const assetPublicPath = config_1.environment.isElectron
        ? path.resolve(config_1.environment.gauzyUserPath, 'public') // Electron-specific path
