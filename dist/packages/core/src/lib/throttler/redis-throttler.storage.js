...
exports.RedisThrottlerStorage = exports.REDIS_THROTTLER_TIMEOUT_MS = void 0;
exports.createThrottlerStorage = createThrottlerStorage;
 ...
 */
exports.REDIS_THROTTLER_TIMEOUT_MS = 250;
/**
...
class RedisThrottlerStorage {
    constructor(client) {
        this.client = client;
        this.logger = new common_1.Logger(RedisThrottlerStorage.name);
        this.fallback = new throttler_1.ThrottlerStorageService();
    }
    /**
    ...
    withDeadline(operation) {
        ...
        return Promise.race([
            ...
            new Promise((_, reject) => {
                timer = setTimeout(() => reject(new Error(`Redis did not answer within ${exports.REDIS_THROTTLER_TIMEOUT_MS}ms`)), exports.REDIS_THROTTLER_TIMEOUT_MS);
                timer.unref?.();
            })
     ...
     */
    async incrementInRedis(hitKey, blockKey, ttl, limit, blockDuration) {
 ...
 */
function createThrottlerStorage(client) {
    if (!client) {
        // eslint-disable-next-line no-console
        console.warn('Throttle buckets are per-process: with more than one API replica the effective rate limit ' +
