...
let ScheduleTimeLogEntriesHandler = class ScheduleTimeLogEntriesHandler {
    constructor(typeOrmTimeLogRepository, mikroOrmTimeLogRepository) {
     ...
     */
    async getPendingTimeLogs(tenantId, organizationId, employeeId) {
        switch (this.ormType) {
            case utils_2.MultiORMEnum.MikroORM: {
                ...
                console.log(`Schedule Time Log Query For ${employeeId ? 'Tenant Organization' : 'All'} Entries`, results.length);
                if (results.length === 0)
                    return [];
                // Re-fetch as entities with timeSlots populated (matching TypeORM's relations: { timeSlots: true })
                const ids = results.map((r) => r.id);
                return await this.mikroOrmTimeLogRepository.find({ id: { $in: ids } }, { populate: ['timeSlots'] });
            }
            ...
            default: {
                ...
                query.where((qb) => {
                    const andWhere = new typeorm_1.Brackets((web) => {
                    ...
                    const orWhere = new typeorm_1.Brackets((web) => {
                ...
                console.log(`Schedule Time Log Query For ${employeeId ? 'Tenant Organization' : 'All'} Entries`, query.getQueryAndParameters());
     ...
     */
    async processTimeLogEntry(timeLog) {
     ...
     */
    async updateStoppedAtUsingStartedAt(timeLog) {
     ...
     */
    async updateStoppedAtUsingTimeSlots(timeLog, timeSlots) {
        // Calculate the total duration in seconds from all time slots
        const totalDurationInSeconds = timeSlots.reduce((sum, { duration }) => sum + duration, 0);
        // Example:
        ...
        const lastTimeSlot = timeSlots.sort((a, b) => moment(b.startedAt).diff(a.startedAt))[0];
        // Example:
     ...
     * @param timeSlots The time slots associated with the time log
     */
    async updateStoppedAtUsingTimeSlots2(timeLog, timeSlots) {
     ...
     */
    async saveTimeLog(partial) {
        switch (this.ormType) {
            case utils_2.MultiORMEnum.MikroORM: {
                const em = this.mikroOrmTimeLogRepository.getEntityManager();
                const entity = em.getReference('TimeLog', partial.id);
...
exports.ScheduleTimeLogEntriesHandler = ScheduleTimeLogEntriesHandler;
exports.ScheduleTimeLogEntriesHandler = ScheduleTimeLogEntriesHandler = tslib_1.__decorate([
