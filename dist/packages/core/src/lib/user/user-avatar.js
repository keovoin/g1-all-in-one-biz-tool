...
exports.SEED_AVATARS_DIR = void 0;
exports.getSeedAvatarFileName = getSeedAvatarFileName;
exports.isSelfResolvingImageUrl = isSelfResolvingImageUrl;
exports.shouldSeedAvatar = shouldSeedAvatar;
/** Directory under `apps/api/src/assets/seed` (and under the API public path) holding the avatars. */
exports.SEED_AVATARS_DIR = 'avatars';
 ...
 */
function getSeedAvatarFileName(imageUrl) {
    if (!imageUrl)
        return undefined;
    ...
    if (HAS_SCHEME.test(value) || value.startsWith('//'))
        return undefined;
    const relative = value.replace(/^\/+/, '');
    if (!relative.startsWith(SEED_AVATAR_PREFIX))
        return undefined;
 ...
 */
function isSelfResolvingImageUrl(url) {
    if (!url)
        return false;
 ...
 */
function shouldSeedAvatar(existingUser) {
    if (!existingUser)
        return true;
    ...
    if (imageUrl && !getSeedAvatarFileName(imageUrl))
        return false;
    return true;
}
