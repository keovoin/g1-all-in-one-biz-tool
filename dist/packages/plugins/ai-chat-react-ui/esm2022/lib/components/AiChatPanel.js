...
import { AI_CHAT_RATE_LIMIT_CODE, AI_CHAT_SETTINGS_PATH, PermissionsEnum } from '@gauzy/contracts';
...
import { buildAttachmentPreamble } from './attachment-preamble';
...
export function AiChatPanel() {
     ...
     */
    const canOpenAiSettings = useCallback(() => (store.userRolePermissions ?? []).some((rolePermission) => rolePermission.permission === PermissionsEnum.AI_CHAT_SETTINGS && rolePermission.enabled), [store]);
    /**
     ...
     */
    const openAiSettings = useCallback((settingsPath) => {
        void injector
    ...
    const chat = useChat({
        ...
        onToolCall: ({ toolCall }) => {
            ...
            executeClientTool(injector, toolName, toolInput)
                .then((output) => chat.addToolOutput({ tool: toolName, toolCallId, output: output }))
        ...
        onError: (streamError) => handleStreamErrorRef.current?.(streamError)
    });
    ...
    handleStreamErrorRef.current = (streamError) => {
        ...
        const wait = envelope.retryAfterSeconds
            ? t('AI_ASSISTANT.RATE_LIMIT_RETRY_IN', 'You can try again in about {{seconds}}s.').replace('{{seconds}}', String(envelope.retryAfterSeconds))
            : '';
        const notice = onSharedKey
            ? t('AI_ASSISTANT.RATE_LIMITED_SHARED', 'The free AI tier is rate limited right now. Connect your own OpenRouter account, or configure a different AI provider, for uninterrupted access.')
            : t('AI_ASSISTANT.RATE_LIMITED_OWN', 'Your AI provider is rate limiting requests right now. Check your plan and limits with the provider, or configure a different AI provider.');
        ...
        if (!onSharedKey || rateLimitPageOpenedRef.current)
            return;
     ...
     */
    const handleSubmit = useCallback((override) => {
    ...
    const handleAttachFile = useCallback(async (file) => {
        ...
        try {
            ...
            if (scope.organizationId)
                docsForm.append('organizationId', scope.organizationId);
            if (scope.tenantId)
                docsForm.append('tenantId', scope.tenantId);
            ...
            if (docsResponse.ok) {
                const body = (await docsResponse.json().catch(() => null));
                ...
                if (rejection) {
                    throw new Error(rejection.message || `${t('AI_ASSISTANT.ATTACH_REJECTED', 'The file was rejected')}: ${file.name}`);
                }
                throw new Error(`${t('AI_ASSISTANT.ATTACH_RESPONSE_UNREADABLE', 'The upload response could not be read — check the Documents page before retrying')}: ${file.name}`);
            }
    ...
    const handlePickDocument = useCallback((document) => {
        setAttachments((current) => [
            ...current,
            {
                documentId: document.id,
    ...
    return (_jsxs("div", { ref: rootRef, style: containerStyle, children: [_jsx("style", { children: `
				@keyframes fadeIn {
			...
			` }), _jsxs("div", { style: headerStyle, children: [_jsx("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: chatTheme.accent, strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: { flexShrink: 0 }, children: _jsx("polygon", { points: "12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" }) }), _jsx("span", { style: headerTitleStyle, children: t('AI_ASSISTANT.TITLE', 'AI Assistant') }), _jsxs("span", { style: { marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 2 }, children: [_jsxs("button", { type: "button", onClick: openHistory, className: "gz-ai-chat-head-btn gz-labelled", style: headerBtnLabelledStyle, title: t('AI_ASSISTANT.HISTORY_HINT', 'Browse saved conversations'), "aria-label": t('AI_ASSISTANT.HISTORY_HINT', 'Browse saved conversations'), children: [_jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: { flexShrink: 0 }, children: [_jsx("circle", { cx: "12", cy: "12", r: "10" }), _jsx("polyline", { points: "12 6 12 12 16 14" })] }), _jsx("span", { className: "gz-ai-chat-btn-label", children: t('AI_ASSISTANT.HISTORY', 'History') })] }), hasMessages && (_jsxs("button", { type: "button", onClick: handleNewChat, className: "gz-ai-chat-head-btn gz-labelled", style: headerBtnLabelledStyle, title: t('AI_ASSISTANT.NEW_CHAT_HINT', 'Start a new conversation'), "aria-label": t('AI_ASSISTANT.NEW_CHAT_HINT', 'Start a new conversation'), children: [_jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: { flexShrink: 0 }, children: [_jsx("path", { d: "M12 20h9" }), _jsx("path", { d: "M16.376 3.622a1 1 0 0 1 3.002 3.002L7.368 18.635a2 2 0 0 1-.855.506l-2.872.838a.5.5 0 0 1-.62-.62l.838-2.872a2 2 0 0 1 .506-.854z" })] }), _jsx("span", { className: "gz-ai-chat-btn-label", children: t('AI_ASSISTANT.NEW_CHAT', 'New chat') })] })), !isDetachedView && (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", onClick: handleMoveSide, className: "gz-ai-chat-head-btn", style: headerBtnStyle, title: dockSide === 'start'
                                            ? t('AI_ASSISTANT.DOCK_RIGHT', 'Dock to the right')
                                            : t('AI_ASSISTANT.DOCK_LEFT', 'Dock to the left'), "aria-label": dockSide === 'start'
                                            ? t('AI_ASSISTANT.DOCK_RIGHT', 'Dock to the right')
                                            : t('AI_ASSISTANT.DOCK_LEFT', 'Dock to the left'), children: dockSide === 'start' ? (_jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("line", { x1: "3", y1: "12", x2: "15", y2: "12" }), _jsx("polyline", { points: "10 7 15 12 10 17" }), _jsx("line", { x1: "20", y1: "4", x2: "20", y2: "20" })] })) : (_jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("line", { x1: "21", y1: "12", x2: "9", y2: "12" }), _jsx("polyline", { points: "14 7 9 12 14 17" }), _jsx("line", { x1: "4", y1: "4", x2: "4", y2: "20" })] })) }), _jsx("button", { type: "button", onClick: handleToggleMaximize, className: "gz-ai-chat-head-btn", style: headerBtnStyle, title: isMaximized
                                            ? t('AI_ASSISTANT.RESTORE', 'Restore width')
                                            : t('AI_ASSISTANT.MAXIMIZE', 'Maximize'), "aria-label": isMaximized
                                            ? t('AI_ASSISTANT.RESTORE', 'Restore width')
                                            : t('AI_ASSISTANT.MAXIMIZE', 'Maximize'), "aria-pressed": isMaximized, children: isMaximized ? (_jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("polyline", { points: "4 14 10 14 10 20" }), _jsx("polyline", { points: "20 10 14 10 14 4" }), _jsx("line", { x1: "14", y1: "10", x2: "21", y2: "3" }), _jsx("line", { x1: "3", y1: "21", x2: "10", y2: "14" })] })) : (_jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("polyline", { points: "15 3 21 3 21 9" }), _jsx("polyline", { points: "9 21 3 21 3 15" }), _jsx("line", { x1: "21", y1: "3", x2: "14", y2: "10" }), _jsx("line", { x1: "3", y1: "21", x2: "10", y2: "14" })] })) }), _jsx("button", { type: "button", onClick: handleDetach, className: "gz-ai-chat-head-btn", style: headerBtnStyle, title: t('AI_ASSISTANT.DETACH', 'Open in a new window'), "aria-label": t('AI_ASSISTANT.DETACH', 'Open in a new window'), children: _jsxs("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("path", { d: "M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" }), _jsx("polyline", { points: "15 3 21 3 21 9" }), _jsx("line", { x1: "10", y1: "14", x2: "21", y2: "3" })] }) }), _jsx("button", { type: "button", onClick: handleCollapse, className: "gz-ai-chat-head-btn", style: headerBtnStyle, title: t('AI_ASSISTANT.CLOSE', 'Close AI Assistant'), "aria-label": t('AI_ASSISTANT.CLOSE', 'Close AI Assistant'), children: _jsx("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: dockSide === 'start' ? (_jsx("polyline", { points: "15 18 9 12 15 6" })) : (_jsx("polyline", { points: "9 18 15 12 9 6" })) }) })] }))] })] }), _jsxs("div", { style: bodyStyle, children: [showHistory && (_jsx(ChatHistoryPanel, { items: history, loading: historyLoading, activeId: activeConversationId, translate: t, onSelect: handleSelectConversation, onDelete: handleDeleteConversation, onClose: () => setShowHistory(false) })), showAttachPicker && (_jsx(DocsAttachPicker, { apiBaseUrl: environment.API_BASE_URL, headers: authHeaders, scope: attachScope, translate: t, onPick: handlePickDocument, onClose: () => setShowAttachPicker(false) })), hasMessages ? (_jsx(ChatMessageList, { messages: messages, status: status, onApprovalResponse: handleApprovalResponse, onOpenCitation: handleOpenCitation, translate: t })) : (_jsx(ChatWelcome, { translate: t })), error && (_jsxs("div", { style: {
                            padding: '8px 12px',
                            ...
                            alignItems: 'center',
                            gap: 7
                        }, children: [_jsx("span", { children: "\u26A0" }), _jsx("span", { children: t('AI_ASSISTANT.ERROR', 'Something went wrong.') }), _jsx("button", { type: "button", onClick: () => regenerate(), style: {
                                    background: 'none',
                                    border: 'none',
                                    ...
                                    fontFamily: 'inherit',
                                    padding: 0
                                }, children: t('AI_ASSISTANT.RETRY', 'Retry') })] })), (attachments.length > 0 || attachmentError) && (_jsxs("div", { style: {
                            display: 'flex',
                            flexWrap: 'wrap',
                            alignItems: 'center',
                            gap: 5,
                            padding: '10px 12px 0'
                        }, children: [attachments.map((attachment, index) => (_jsxs("span", { style: {
                                    display: 'inline-flex',
                                    alignItems: 'center',
                                    gap: 5,
                                    maxWidth: '100%',
                                    ...
                                    fontWeight: chatTheme.fontWeightMedium,
                                    lineHeight: 1.5
                                }, children: [_jsx("span", { "aria-hidden": "true", children: "\uD83D\uDCCE" }), _jsx("span", { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, title: attachment.name, children: attachment.name }), _jsx("button", { type: "button", onClick: () => setAttachments((current) => current.filter((_entry, entryIndex) => entryIndex !== index)), "aria-label": `${t('AI_ASSISTANT.ATTACH_REMOVE', 'Remove attachment')}: ${attachment.name}`, style: {
                                            background: 'none',
                                            border: 'none',
                                            color: chatTheme.textSecondary,
                                            cursor: 'pointer',
                                            padding: 0,
                                            lineHeight: 1
                                        }, children: "\u00D7" })] }, `${attachment.documentId ?? attachment.name}-${index}`))), attachmentError && (_jsx("span", { style: {
                                    color: chatTheme.red,
                                    fontSize: chatTheme.fontSizeSmall,
                                    lineHeight: 1.5
                                }, children: attachmentError }))] })), _jsx(ChatInput, { value: input, isBusy: isBusy, translate: t, onChange: setInput, onSubmit: handleSubmit, onStop: () => void stop(), onEscape: isDetachedView ? undefined : handleCollapse, onTranscribe: transcribeAudio, onOpenAiSettings: canOpenAiSettings() ? openAiSettings : undefined, onAttachFile: handleAttachFile, onAttachFromDocuments: () => {
                            setAttachmentError(null);
                            setShowAttachPicker(true);
                        }, isAttaching: isAttaching, composingFor: activeConversationId })] }), !isMaximized && !isDetachedView && (_jsx("div", { className: "gz-ai-chat-resize", style: resizeHandleStyle, onPointerDown: handleResizeStart, role: "separator", "aria-orientation": "vertical", title: t('AI_ASSISTANT.RESIZE', 'Drag to resize'), "aria-label": t('AI_ASSISTANT.RESIZE', 'Drag to resize') }))] }));
}
