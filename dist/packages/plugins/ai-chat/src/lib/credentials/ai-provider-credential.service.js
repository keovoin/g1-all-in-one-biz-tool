"use strict";
var AiProviderCredentialService_1;
 ...
 */
let AiProviderCredentialService = AiProviderCredentialService_1 = class AiProviderCredentialService extends core_1.TenantAwareCrudService {
    constructor(typeOrmAiProviderCredentialRepository, mikroOrmAiProviderCredentialRepository, encryptionService) {
        super(typeOrmAiProviderCredentialRepository, mikroOrmAiProviderCredentialRepository);
        this.typeOrmAiProviderCredentialRepository = typeOrmAiProviderCredentialRepository;
        this.mikroOrmAiProviderCredentialRepository = mikroOrmAiProviderCredentialRepository;
        this.encryptionService = encryptionService;
        this.logger = new common_1.Logger(AiProviderCredentialService_1.name);
    }
    /**
     ...
     */
    async getDecryptedCredential(providerId, tenantId) {
        ...
        if (record.baseUrl) {
            const reason = (0, ssrf_1.getUnsafeAiProviderBaseUrlReason)(record.baseUrl);
            if (reason) {
                // Never logs the URL: this line ends up in shared log storage and the host is exactly
                // what the tenant was probing for.
                this.logger.warn(`Ignoring stored credential for provider '${providerId}' (tenant '${tenantId}'): ` +
        ...
        try {
            ...
            this.logger.warn(`Failed to decrypt API key for provider '${providerId}' (tenant '${tenantId}'). ` +
     ...
     */
    async getTenantDefault(tenantId) {
        const { success, record } = await this.findOneOrFailByOptions({
     ...
     */
    async getTenantVoiceDefault(tenantId) {
        const { success, record } = await this.findOneOrFailByOptions({
            where: { tenantId, enabled: true, isVoiceDefault: true }
        });
        if (!success || !record) {
            return null;
        }
    ...
    async connectExchange(input) {
        ...
        if (definition.connect?.type !== 'openrouter-pkce') {
            throw new common_1.BadRequestException(`Provider '${providerId}' does not support a Connect flow.`);
        }
        let response;
        try {
            ...
            throw new common_1.BadGatewayException('Could not reach OpenRouter to complete the connection. Please try again.');
        }
        if (!response.ok) {
            const body = await response.text().catch(() => '');
            this.logger.warn(`OpenRouter key exchange rejected (${response.status}): ${body.slice(0, 300)}`);
            throw new common_1.BadGatewayException(`OpenRouter rejected the connection (HTTP ${response.status}). The authorization may have expired — please try connecting again.`);
        }
        const payload = (await response.json().catch(() => null));
        if (!payload?.key) {
            throw new common_1.BadGatewayException('OpenRouter did not return an API key for this authorization.');
        }
    ...
    async upsert(input) {
        ...
        if (input.baseUrl) {
            (0, base_url_validator_1.assertSafeAiProviderBaseUrl)(input.baseUrl);
        }
        ...
        const payload = { ...input, providerId, tenantId };
        if (input.apiKey) {
        ...
        if (existing) {
            await super.update(existing.id, payload);
     ...
     */
    assertKeyOptional(providerId) {
        const definition = provider_registry_1.AiProviderRegistry.get(providerId);
        if (definition?.requiresApiKey === false) {
            return;
        }
        throw new common_1.BadRequestException(definition
     ...
     */
    assertBaseUrlSatisfied(providerId, baseUrl) {
        const definition = provider_registry_1.AiProviderRegistry.get(providerId);
        ...
        throw new common_1.BadRequestException(`A base URL is required for provider '${definition.label}' — enter the address of your server (e.g. http://localhost:8000/v1).`);
    }
    /**
    ...
    async updateCredential(id, input) {
        ...
        const payload = { ...input };
        delete payload.providerId; // provider identity is immutable per credential
        delete payload.tenantId; // never re-parent a credential
        if (input.apiKey) {
            payload.apiKey = this.encryptionService.encrypt(input.apiKey);
        }
        else {
        ...
        this.assertBaseUrlSatisfied(existing.providerId, input.baseUrl === undefined ? existing.baseUrl : (input.baseUrl ?? undefined));
        if (input.isDefault) {
     ...
     */
    async findAllMasked() {
        const { items, total } = await this.findAll({ order: { createdAt: 'DESC' } });
        return {
     ...
     */
    async clearOtherDefaults(tenantId, exceptId) {
        ...
        for (const other of others) {
            ...
            await super.update(other.id, { isDefault: false });
        }
    }
    /**
     ...
     */
    async clearOtherVoiceDefaults(tenantId, exceptId) {
        const others = await this.find({
            where: { tenantId, isVoiceDefault: true }
        });
        for (const other of others) {
            if (exceptId && other.id === exceptId) {
                continue;
            }
            await super.update(other.id, { isVoiceDefault: false });
        }
    }
    /**
...
exports.AiProviderCredentialService = AiProviderCredentialService = AiProviderCredentialService_1 = tslib_1.__decorate([
    (0, common_1.Injectable)(),
    tslib_1.__metadata("design:paramtypes", [type_orm_ai_provider_credential_repository_1.TypeOrmAiProviderCredentialRepository,
        mikro_orm_ai_provider_credential_repository_1.MikroOrmAiProviderCredentialRepository,
        ai_provider_credential_encryption_service_1.AiProviderCredentialEncryptionService])
], AiProviderCredentialService);
