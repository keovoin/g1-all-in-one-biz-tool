...
exports.createCatalogueCache = createCatalogueCache;
exports.credentialCacheKey = credentialCacheKey;
exports.fetchCatalogueJson = fetchCatalogueJson;
exports.publicCatalogue = publicCatalogue;
exports.keyedCatalogue = keyedCatalogue;
exports.selfHostedCatalogue = selfHostedCatalogue;
exports.mergeCatalogue = mergeCatalogue;
exports.prettifyModelId = prettifyModelId;
...
function createCatalogueCache(options) {
 ...
 */
async function fetchCatalogueJson(url, init) {
    const response = await (0, ssrf_1.ssrfSafeFetch)(url, {
 ...
 */
async function publicCatalogue(options) {
    try {
 ...
 */
async function keyedCatalogue(options) {
    const { credentials, curated } = options;
    if (!credentials?.apiKey || credentials.baseUrl)
        return { models: curated, source: 'curated' };
    try {
        const { value, stale } = await options.cache.get(credentialCacheKey(credentials), () => options.load(credentials));
 ...
 */
async function selfHostedCatalogue(options) {
    const { credentials, curated } = options;
 ...
 */
function mergeCatalogue(curated, fetched) {
