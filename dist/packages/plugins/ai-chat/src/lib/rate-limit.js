...
exports.buildRateLimitEnvelope = exports.rateLimitRetryAfter = exports.isRateLimitError = exports.RATE_LIMIT_CODE = void 0;
const contracts_1 = require("@gauzy/contracts");
Object.defineProperty(exports, "RATE_LIMIT_CODE", { enumerable: true, get: function () { return contracts_1.AI_CHAT_RATE_LIMIT_CODE; } });
...
const isRateLimitError = (error) => {
    for (const candidate of unwrap(error)) {
        const e = candidate;
        if (Number(e.statusCode) === 429 || Number(e.status) === 429 || Number(e.code) === 429)
            return true;
        if (Number(e.data?.error?.code) === 429)
            return true;
    }
    return false;
};
...
const rateLimitRetryAfter = (error) => {
    for (const candidate of unwrap(error)) {
...
exports.rateLimitRetryAfter = rateLimitRetryAfter;
/**
 ...
 * parses it back out.
 */
const buildRateLimitEnvelope = (envelope) => JSON.stringify(envelope);
