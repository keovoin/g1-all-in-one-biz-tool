...
exports.createSsrfSafeLookup = createSsrfSafeLookup;
exports.ssrfSafeFetch = ssrfSafeFetch;
exports.createAiProviderSdkFetch = createAiProviderSdkFetch;
exports.isSsrfBlockedError = isSsrfBlockedError;
const dns_1 = require("dns");
const net_1 = require("net");
const utils_1 = require("@gauzy/utils");
const fetch_over_node_http_1 = require("./fetch-over-node-http");
const outbound_url_guard_1 = require("./outbound-url-guard");
...
const defaultResolver = (hostname) => new Promise((resolve, reject) => {
    (0, dns_1.lookup)(hostname, { all: true, verbatim: true }, (error, addresses) => {
        if (error)
            return reject(error);
 ...
 */
async function assertResolvedHostIsPublic(hostname, resolver, signal) {
    let addresses;
    try {
        ...
        if (signal?.aborted && error === signal.reason)
            throw error;
        if (isNonExistentHostError(error))
            return;
    ...
    if (addresses.some((address) => (0, utils_1.isPrivateOrLoopbackHost)(address))) {
        throw blockedError();
    }
}
/** Whether a resolver error is the definitive "no such host" verdict ({@link NON_EXISTENT_HOST_CODES}). */
function isNonExistentHostError(error) {
 ...
 */
const blockedError = () => new SsrfBlockedError('The configured AI provider endpoint resolves to a non-public address.');
/** The fail-closed refusal for a lookup that failed without a verdict. Names nothing, like {@link blockedError}. */
const unverifiableError = () => new SsrfBlockedError('The configured AI provider endpoint could not be verified as a public address.');
/** Address family a `lookup` caller asked for; `0` for any. */
function requestedFamily(family) {
...
function resolveForConnection(hostname, options, resolver) {
    if (resolver) {
        return resolver(hostname).then((addresses) => (addresses ?? []).map((address) => ({ address, family: (0, net_1.isIP)(address) })));
    }
    return new Promise((resolve, reject) => {
        (0, dns_1.lookup)(hostname, { hints: options.hints, all: true }, (error, addresses) => {
            if (error)
                return reject(error);
            resolve(addresses ?? []);
        });
 ...
 * @param resolver - Resolver override (tests); `dns.lookup` otherwise.
 */
function createSsrfSafeLookup(allowPrivateHost, resolver) {
    return (hostname, options, callback) => {
        const done = callback;
        resolveForConnection(hostname, options, resolver)
            .then((entries) => {
            if (!allowPrivateHost && entries.some((entry) => (0, utils_1.isPrivateOrLoopbackHost)(entry.address))) {
            ...
            if (usable.length === 0) {
                return done(Object.assign(new Error(`getaddrinfo ENOTFOUND ${hostname}`), {
                    code: 'ENOTFOUND',
                    syscall: 'getaddrinfo',
                    hostname
                }));
            ...
            if (options.all)
                return done(null, usable);
            return done(null, usable[0].address, usable[0].family);
        }, (error) => {
            if (allowPrivateHost || isNonExistentHostError(error))
                return done(error);
...
async function guardedFetch(input, init, options) {
    ...
    const allowPrivateHost = options?.allowPrivateHost ?? (0, outbound_url_guard_1.isPrivateAiProviderBaseUrlAllowed)();
    ...
    try {
        return await (0, fetch_over_node_http_1.fetchOverNodeHttp)(input, init, {
 ...
 */
async function assertOutboundTargetAllowed(url, signal, allowPrivate, resolver) {
    const reason = (0, outbound_url_guard_1.getUnsafeAiOutboundUrlReason)(url, { allowPrivate });
    if (reason) {
