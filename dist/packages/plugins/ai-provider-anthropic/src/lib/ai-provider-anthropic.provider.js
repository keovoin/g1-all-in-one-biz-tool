...
exports.anthropicProviderDefinition = void 0;
...
const PROVIDER_ID = contracts_1.AiProviderEnum.ANTHROPIC;
/**
...
const listCatalogue = async (credentials) => (0, plugin_ai_chat_1.keyedCatalogue)({
    ...
    load: async (resolved) => {
        const body = await (0, plugin_ai_chat_1.fetchCatalogueJson)('https://api.anthropic.com/v1/models?limit=1000', { headers: { 'x-api-key': resolved.apiKey, 'anthropic-version': '2023-06-01' } });
        return (body.data ?? [])
            .filter((m) => typeof m?.id === 'string' && !LEGACY_MODEL_PATTERN.test(m.id))
            .map((m) => ({
            id: m.id,
            label: m.display_name ?? (0, plugin_ai_chat_1.prettifyModelId)(m.id),
 ...
 */
exports.anthropicProviderDefinition = {
    id: PROVIDER_ID,
    label: 'Anthropic',
    ...
    async createModel(modelId, credentials) {
        const { createAnthropic } = await (0, plugin_ai_chat_1.importEsm)('@ai-sdk/anthropic');
...
//# sourceMappingURL=ai-provider-anthropic.provider.js.map