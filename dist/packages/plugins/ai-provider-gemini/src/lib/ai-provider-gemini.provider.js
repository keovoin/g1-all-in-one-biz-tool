...
exports.geminiProviderDefinition = void 0;
...
const PROVIDER_ID = contracts_1.AiProviderEnum.GEMINI;
/**
 * Chat models offered by Gemini (shown in the model selector).
 * Model ids as accepted by `@ai-sdk/google`.
 */
const MODELS = [{ id: 'gemini-3.5-flash', label: 'Gemini 3.5 Flash', providerId: PROVIDER_ID }];
/**
...
const listCatalogue = async (credentials) => (0, plugin_ai_chat_1.keyedCatalogue)({
    ...
    load: async (resolved) => {
        const body = await (0, plugin_ai_chat_1.fetchCatalogueJson)('https://generativelanguage.googleapis.com/v1beta/models?pageSize=1000', {
        ...
        const fetched = (body.models ?? [])
            ...
            .map((m) => ({
            id: m.id,
            label: m.displayName ?? (0, plugin_ai_chat_1.prettifyModelId)(m.id),
        ...
        return fetched.length ? (0, plugin_ai_chat_1.mergeCatalogue)(MODELS, fetched) : [];
    }
});
/**
 ...
 */
exports.geminiProviderDefinition = {
    id: PROVIDER_ID,
    label: 'Gemini',
    ...
    async createModel(modelId, credentials) {
        const { createGoogleGenerativeAI } = await (0, plugin_ai_chat_1.importEsm)('@ai-sdk/google');
     ...
     */
    async createEmbeddingModel(modelId, credentials) {
        const { createGoogleGenerativeAI } = await (0, plugin_ai_chat_1.importEsm)('@ai-sdk/google');
        const provider = createGoogleGenerativeAI({
            apiKey: credentials.apiKey,
            ...(credentials.baseUrl ? { baseURL: credentials.baseUrl } : {}),
...
//# sourceMappingURL=ai-provider-gemini.provider.js.map