...
exports.openRouterProviderDefinition = void 0;
...
const PROVIDER_ID = contracts_1.AiProviderEnum.OPENROUTER;
/**
...
const listFreeModels = async () => {
    ...
    if (cached && Date.now() - cached.fetchedAt < FREE_MODEL_TTL_MS)
    ...
    freeModelInFlight = (async () => {
        try {
            ...
            if (!models.length)
                throw new Error('OpenRouter /models listed no tool-capable :free models');
 ...
 */
const listCatalogue = async () => (0, plugin_ai_chat_1.publicCatalogue)({
    curated: MODELS,
    cache: catalogueCache,
    load: async () => {
        const body = await (0, plugin_ai_chat_1.fetchCatalogueJson)(`https://openrouter.ai/api/v1/models?supported_parameters=${TOOLS_PARAMETER}`);
        return (body.data ?? [])
 ...
 * module load time.
 */
exports.openRouterProviderDefinition = {
    id: PROVIDER_ID,
    label: 'OpenRouter',
    ...
    async createModel(modelId, credentials) {
        ...
        if (credentials.source === 'platform' && !modelId.endsWith(FREE_SUFFIX)) {
            throw new Error(`Refusing to use '${modelId}' on the shared free-tier key: only ${FREE_SUFFIX} models are permitted.`);
        }
        const { createOpenRouter } = await (0, plugin_ai_chat_1.importEsm)('@openrouter/ai-sdk-provider');
        ...
        return provider.chat(modelId, settings);
    }
};
//# sourceMappingURL=ai-provider-openrouter.provider.js.map