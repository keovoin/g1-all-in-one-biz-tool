 ...
 */
export function resolveRangePeriod(context) {
 ...
 */
export function isCurrentWeekRange(context) {
    if (!context?.startDate || !context?.endDate) {
        return false;
    }
    return (moment(context.startDate).format('YYYY-MM-DD') === moment().startOf('week').format('YYYY-MM-DD') &&
 ...
 */
export function isMoreThanWeekRange(context) {
    if (!context?.startDate || !context?.endDate) {
        return false;
    }
 ...
 */
export function resolvePeriodSeconds(context, employeesCount) {
    if (!context?.startDate || !context?.endDate) {
        return 0;
    }
 ...
 */
function resolveWorkingSeconds(context) {
 ...
 */
export function timeTrackScopeKey(context) {
    if (!context) {
        return '';
    }
    return [
 ...
 */
export function rangeMessageKey(baseKey, period) {
 ...
 */
export function toWeekHourBars(weekHours) {
    const byDay = new Map();
    let total = 0;
...
//# sourceMappingURL=time-track-widget.utils.js.map