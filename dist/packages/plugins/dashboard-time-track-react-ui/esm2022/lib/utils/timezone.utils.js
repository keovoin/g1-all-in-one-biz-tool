 ...
 */
export function getTimeZoneWithOffset(zone, at = moment()) {
    let region = '';
    let city = '';
    if (zone.includes('/')) {
 ...
 */
export function resolveMomentTimezone(zone, sources, guess = () => moment.tz.guess()) {
    switch (zone) {
 ...
 */
export function normalizeTimeZoneOption(zone) {
    switch (zone) {
        case TimeZoneEnum.ORG_TIMEZONE:
        case TimeZoneEnum.MINE_TIMEZONE:
            return zone;
