...
export const Callout = Node.create({
    ...
    addAttributes() {
        return {
            ...
            emoji: {
                ...
                renderHTML: (attributes) => attributes['emoji'] ? { 'data-callout-emoji': attributes['emoji'] } : {}
            }
        };
    },
    parseHTML() {
    ...
    addCommands() {
        return {
            setCallout: (attributes = {}) => ({ commands }) => commands.wrapIn(this.name, { type: 'info', emoji: null, ...attributes }),
            unsetCallout: () => ({ commands }) => commands.lift(this.name)
        };
    }
});
//# sourceMappingURL=callout.node.js.map