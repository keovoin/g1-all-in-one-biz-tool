...
const ALLOWED_URL_SCHEMES = new Set(['http', 'https', 'mailto', 'tel', 'blob']);
/**
 ...
 */
function isAllowedSrcset(value) {
    return value
        .split(',')
        .map((candidate) => candidate.trim().split(/\s+/)[0])
        .filter((url) => url.length > 0)
        .every((url) => isAllowedUrl(url));
}
...
//# sourceMappingURL=safe-url.util.js.map