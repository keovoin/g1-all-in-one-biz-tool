const chain = (editor, range) => editor.chain().focus().deleteRange(range);
/**
...
export const SLASH_COMMANDS = [
    ...
    },
    {
        ...
        action: (editor, range) => void chain(editor, range).insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()
    },
    {
        id: 'image',
    ...
    },
    {
        ...
        action: (editor, range, deps) => {
            ...
            void deps
                ...
                .then((url) => {
                if (url)
                    editor.chain().focus().insertEmbedCard({ url }).run();
            })
                .catch(() => undefined);
        }
    },
    ...
    },
    {
        ...
        action: (editor, range) => void chain(editor, range)
...
export function filterSlashCommands(query, translate, deps) {
...
//# sourceMappingURL=slash-menu.items.js.map