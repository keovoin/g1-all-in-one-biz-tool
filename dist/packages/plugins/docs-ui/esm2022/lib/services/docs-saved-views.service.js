...
import { DOCS_SAVED_VIEWS_KEY_PREFIX, DOCS_SAVED_VIEWS_LIMIT, DOCS_SAVED_VIEW_NAME_MAX } from '../docs.constants';
import { DOCS_SAVED_VIEW_EXCLUDED_PARAMS, DOCS_SAVED_VIEW_PARAMS } from '../models/docs-saved-view.model';
...
export class DocsSavedViewsService {
    constructor(store) {
        ...
        this.views$ = this._views$.asObservable();
    ...
    refresh() {
        ...
        if (this.loadedOrganizationId === organizationId && this._views$.value.length)
            return;
    ...
    save(name, params) {
        ...
        if (existingIndex >= 0) {
            ...
            if (views.length >= DOCS_SAVED_VIEWS_LIMIT)
                return null;
    ...
    rename(id, name) {
        ...
        if (views.some((view) => view.id !== id && view.name.toLowerCase() === label.toLowerCase()))
            return false;
        this.write(views.map((view) => (view.id === id ? { ...view, name: label, updatedAt: new Date().toISOString() } : view)));
        return true;
    }
    remove(id) {
    ...
    matches(view, params) {
        ...
        for (const key of keys) {
            if (String(current[key] ?? '') !== String(view.params[key] ?? ''))
                return false;
        }
        return true;
    }
    ...
    captureParams(params) {
        const excluded = new Set(DOCS_SAVED_VIEW_EXCLUDED_PARAMS);
        const captured = {};
        for (const key of DOCS_SAVED_VIEW_PARAMS) {
            if (excluded.has(key))
                continue;
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocsSavedViewsService, deps: [{ token: i1.Store }], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocsSavedViewsService }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DocsSavedViewsService, decorators: [{
            type: Injectable
        }], ctorParameters: () => [{ type: i1.Store }] });
//# sourceMappingURL=docs-saved-views.service.js.map