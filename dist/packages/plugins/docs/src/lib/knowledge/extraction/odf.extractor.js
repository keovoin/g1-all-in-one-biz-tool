...
exports.OdfExtractor = exports.ODS_MAX_COLUMNS_PER_ROW = exports.ODS_MIME_TYPE = exports.ODT_MIME_TYPE = void 0;
...
exports.ODT_MIME_TYPE = 'application/vnd.oasis.opendocument.text';
/** Canonical MIME of an OpenDocument spreadsheet (`.ods`). */
exports.ODS_MIME_TYPE = 'application/vnd.oasis.opendocument.spreadsheet';
/**
 ...
 */
exports.ODS_MAX_COLUMNS_PER_ROW = 256;
 ...
 */
let OdfExtractor = class OdfExtractor {
    /**
     * @inheritdoc
     */
    supports(mime) {
        return mime === exports.ODT_MIME_TYPE || mime === exports.ODS_MIME_TYPE;
    }
    /**
     * @inheritdoc
     */
    async extract(buffer, ctx) {
        ...
        if (!xml) {
            throw new errors_1.DocsPermanentError('The document could not be read — it is not an OpenDocument package.');
        }
        const content = (0, office_xml_util_1.parseXml)(xml);
        const warnings = [];
        const isSpreadsheet = ctx.mimeType === exports.ODS_MIME_TYPE;
        const { markdown: rendered, pageCount } = isSpreadsheet
            ? this.renderSpreadsheet(content, warnings)
            : this.renderText(content);
        const normalized = (0, extractor_interface_1.normalizeMarkdown)(rendered || '_Empty document._');
    ...
    renderText(content) {
        ...
        return { markdown: (0, office_markdown_util_1.joinBlocks)(this.renderBlocks(body, 0)), pageCount: 1 };
    }
    /**
     ...
     */
    renderBlocks(node, listDepth) {
        const lines = [];
        for (const child of node.children) {
            switch (child.localName) {
                case 'h': {
                    const level = Math.min(Math.max(Number((0, office_xml_util_1.attribute)(child, 'outline-level') ?? 1) || 1, 1), MAX_HEADING_LEVEL);
                ...
                case 'list-item':
                    ...
                    break;
                case 'table':
                    lines.push('', (0, office_markdown_util_1.renderPipeTable)(this.tableRows(child, xlsx_extractor_1.XLSX_MAX_ROWS_PER_SHEET).rows), '');
                    break;
                case 'section':
                case 'frame':
                case 'text-box':
     ...
     */
    renderSpreadsheet(content, warnings) {
        ...
        for (const sheet of (0, office_xml_util_1.findAll)(body, 'table')) {
            const name = (0, office_xml_util_1.attribute)(sheet, 'name') ?? `Sheet ${nonEmptySheets + 1}`;
            const { rows, capped, totalRows } = this.tableRows(sheet, xlsx_extractor_1.XLSX_MAX_ROWS_PER_SHEET);
            if (!rows.length) {
                continue; // empty sheet — not counted, not rendered
            }
            nonEmptySheets++;
            const lines = [`## Sheet: ${name}`, '', (0, office_markdown_util_1.renderPipeTable)(rows)];
            if (capped) {
                lines.push('', `_Truncated: only the first ${xlsx_extractor_1.XLSX_MAX_ROWS_PER_SHEET} of ${totalRows} rows are shown._`);
                warnings.push(`Sheet "${name}" row cap applied (${xlsx_extractor_1.XLSX_MAX_ROWS_PER_SHEET})`);
            }
     ...
     */
    tableRows(table, maxRows) {
        const rows = [];
        let totalRows = 0;
        let capped = false;
        for (const row of (0, office_xml_util_1.findAll)(table, 'table-row')) {
    ...
    rowCells(row) {
        ...
        for (const cell of row.children) {
            ...
            const repeat = Math.min(this.repeatCount(cell, 'number-columns-repeated'), exports.ODS_MAX_COLUMNS_PER_ROW);
            for (let i = 0; i < repeat && cells.length < exports.ODS_MAX_COLUMNS_PER_ROW; i++) {
                cells.push(value);
            }
        }
    ...
    repeatCount(node, name) {
        const parsed = Number((0, office_xml_util_1.attribute)(node, name) ?? 1);
     ...
     */
    inlineText(node) {
        return (0, office_xml_util_1.textContent)(node, TEXT_REPLACEMENTS).replace(/\s+/g, ' ').trim();
    }
    /**
     ...
     */
    findBody(content, localName) {
        const body = (0, office_xml_util_1.findAll)(content, 'body')[0];
        return body ? body.children.find((child) => child.localName === localName) : undefined;
    }
};
exports.OdfExtractor = OdfExtractor;
exports.OdfExtractor = OdfExtractor = tslib_1.__decorate([
    (0, common_1.Injectable)()
], OdfExtractor);
//# sourceMappingURL=odf.extractor.js.map