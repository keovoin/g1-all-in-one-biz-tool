...
function parseXml(xml) {
    if (!xml || !xml.trim()) {
        throw new errors_1.DocsPermanentError('The document part is empty and could not be read.');
    }
    const root = { name: '#document', localName: '#document', attributes: {}, children: [], content: [] };
    const stack = [root];
    let nodeCount = 0;
    let cursor = 0;
    while (cursor < xml.length) {
        ...
        if (close === -1) {
            throw new errors_1.DocsPermanentError('The document part could not be read — it contains an unterminated tag.');
        }
        ...
        if (++nodeCount > MAX_NODES) {
            throw new errors_1.DocsPermanentError('The document contains too many elements to process.');
        }
        ...
        if (!selfClosing) {
            if (stack.length >= MAX_DEPTH) {
                throw new errors_1.DocsPermanentError('The document is nested too deeply to process.');
            }
            stack.push(node);
        }
    }
 ...
 */
function textContent(node, replacements = {}) {
    let text = '';
    const walk = (current) => {
...
function decodeEntities(text) {
    ...
    return text.replace(/&(#x[0-9a-fA-F]+|#\d+|[a-zA-Z][a-zA-Z0-9]*);/g, (whole, entity) => {
...
function fromCodePoint(codePoint, whole) {
...
//# sourceMappingURL=office-xml.util.js.map