...
const type_orm_document_version_repository_1 = require("../repositories/type-orm-document-version.repository");
let DocumentVersionService = DocumentVersionService_1 = class DocumentVersionService extends core_1.TenantAwareCrudService {
    constructor(typeOrmDocumentVersionRepository, mikroOrmDocumentVersionRepository, typeOrmDocumentRepository) {
        super(typeOrmDocumentVersionRepository, mikroOrmDocumentVersionRepository);
        this.typeOrmDocumentVersionRepository = typeOrmDocumentVersionRepository;
        this.mikroOrmDocumentVersionRepository = mikroOrmDocumentVersionRepository;
        this.typeOrmDocumentRepository = typeOrmDocumentRepository;
        this.logger = new common_1.Logger(DocumentVersionService_1.name);
    }
    /**
     ...
     */
    async captureSnapshotIfNeeded(document, options = {}) {
        ...
        if (!options.force) {
            const { versionDebounceMinutes } = (0, docs_config_1.getDocsConfig)();
            ...
            const recent = await this.typeOrmDocumentVersionRepository.findOne({
                where: {
                    ...
                    createdById: employeeId ? employeeId : (0, typeorm_1.IsNull)(),
                    lastSavedAt: (0, typeorm_1.MoreThan)(windowStart)
                },
        ...
        await this.typeOrmDocumentRepository.update({ id: document.id, tenantId: document.tenantId }, { version: document.version });
        return version;
    }
    /**
     ...
     */
    async getVersion(document, versionId) {
        ...
        if (!version) {
            throw new common_1.NotFoundException(`Document version ${versionId} was not found`);
        }
        return version;
    }
    /**
     ...
     */
    async restoreVersion(document, versionId) {
        if (document.kind !== contracts_1.DocumentKindEnum.PAGE) {
            throw new common_1.ConflictException({ message: 'Versions apply to PAGE documents only', code: docs_constants_1.DOCS_NOT_A_PAGE });
        }
...
exports.DocumentVersionService = DocumentVersionService = DocumentVersionService_1 = tslib_1.__decorate([
    (0, common_1.Injectable)(),
    tslib_1.__metadata("design:paramtypes", [type_orm_document_version_repository_1.TypeOrmDocumentVersionRepository,
        mikro_orm_document_version_repository_1.MikroOrmDocumentVersionRepository,
