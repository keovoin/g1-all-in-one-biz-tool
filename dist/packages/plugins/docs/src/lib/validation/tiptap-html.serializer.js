...
const tiptap_schema_validator_1 = require("./tiptap-schema.validator");
/**
...
function applyMarks(text, marks) {
    ...
    return marks.reduce((inner, mark) => {
        ...
        if (typeof type !== 'string' || !Object.prototype.hasOwnProperty.call(tiptap_schema_validator_1.DOCS_SCHEMA_MARKS, type)) {
            return inner;
        }
...
function renderNode(value) {
    ...
    if (typeof type !== 'string' || !Object.prototype.hasOwnProperty.call(tiptap_schema_validator_1.DOCS_SCHEMA_NODES, type)) {
    ...
    switch (type) {
        ...
        case 'documentMention':
            return `<a${renderAttributes({ href: `/pages/documents?id=${attrs['id']}` })}>${escapeHtml(attrs['label'] ?? attrs['id'])}</a>`;
        case 'tableHeader':
        case 'tableCell':
 ...
 */
function generateDocumentHtml(contentJson) {
    return renderNode(contentJson);
}
//# sourceMappingURL=tiptap-html.serializer.js.map