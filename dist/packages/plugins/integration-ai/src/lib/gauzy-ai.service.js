...
const gauzy_ai_sdk_1 = require("./sdk/gauzy-ai-sdk");
...
const configuration_interface_1 = require("./configuration.interface");
let GauzyAIService = GauzyAIService_1 = class GauzyAIService {
    constructor(_configService, _http, _requestConfigProvider) {
     ...
     */
    sendRequest(path, options = {}, method = configuration_interface_1.HttpMethodEnum.GET, defaultHeaders = {
        ...
        const customHeaders = () => ({
            ...
            'X-APP-ID': this._configService.get('gauzyAI.gauzyAiApiKey'),
            'X-API-KEY': this._configService.get('gauzyAI.gauzyAiApiSecret'),
            /** */
        ...
        try {
            ...
            if (error.response) {
                ...
                throw new common_1.HttpException('An error occurred while making the request.', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }
    }
    /**
     ...
     */
    async analyzeImage(stream, file) {
        ...
        return await (0, rxjs_1.firstValueFrom)(this.sendRequest('image/process', options, configuration_interface_1.HttpMethodEnum.POST, headers).pipe((0, operators_1.map)((resp) => resp.data)));
    }
    /**
    ...
    async preProcessEmployeeJobApplication(params) {
        ...
        // Call the sendRequest function with the appropriate parameters
        return await (0, rxjs_1.firstValueFrom)(this.sendRequest('employee/job/application/pre-process', {
            data: params // Set the request payload
        }, configuration_interface_1.HttpMethodEnum.POST).pipe((0, operators_1.tap)((resp) => console.log(resp)), (0, operators_1.map)((resp) => resp.data)));
    }
    /**
     ...
     * @returns
     */
    async generateAIProposalForEmployeeJobApplication(employeeJobApplicationId) {
        // Call the sendRequest function with the appropriate parameters
        return await (0, rxjs_1.firstValueFrom)(this.sendRequest(`employee/job/application/generate-proposal/${employeeJobApplicationId}`, {}, configuration_interface_1.HttpMethodEnum.POST).pipe((0, operators_1.tap)((resp) => console.log(resp)), (0, operators_1.map)((resp) => resp.data)));
    }
    /**
    ...
    async getEmployeeJobApplication(employeeJobApplicationId) {
        // Call the sendRequest function with the appropriate parameters
        return await (0, rxjs_1.firstValueFrom)(this.sendRequest(`employee/job/application/${employeeJobApplicationId}`).pipe((0, operators_1.tap)((resp) => console.log(resp)), (0, operators_1.map)((resp) => resp.data)));
    }
    /**
     ...
     */
    async updateEmployeeStatus({ employeeId, tenantId, organizationId, isJobSearchActive }) {
        if (this._client == null) {
            return false;
        }
        // First we need to get employee id because we have only externalId
        const gauzyAIEmployeeId = await this.getEmployeeGauzyAIId(employeeId);
        console.log(`updateEmployeeStatus called. EmployeeId: ${employeeId}. Gauzy AI EmployeeId: ${gauzyAIEmployeeId}`);
        const update = {
        ...
        const updateEmployeeMutation = (0, core_1.gql) `
    ...
    async apply(input) {
        ...
        if (employeeId && jobPostId && employeeJobPostId) {
            ...
            const createOneEmployeeJobApplication = {
            ...
            // Call the sendRequest function with the appropriate parameters
            const response = await (0, rxjs_1.firstValueFrom)(this.sendRequest(`employee/job/application/process`, {
                method: configuration_interface_1.HttpMethodEnum.POST, // Set the HTTP method to GET
    ...
    async updateVisibility(input) {
        ...
        if (input.employeeId && input.providerCode && input.providerJobId) {
            ...
            if (employeeId && jobPostId && employeeJobPostId) {
                ...
                const updateEmployeeJobPostMutation = (0, core_1.gql) `
    ...
    async updateApplied(input) {
        ...
        if (employeeId && jobPostId && employeeJobPostId) {
            ...
            const createOneEmployeeJobApplicationMutation = (0, core_1.gql) `
    ...
    async syncGauzyEmployeeJobSearchCriteria(employee, criteria) {
        ...
        try {
            const gauzyAIUser = await this.syncUser({
            ...
            const deleteAllCriteriaMutation = (0, core_1.gql) `
            ...
            console.log(`Delete Existed Criterions count: ${JSON.stringify(deleteMutationResult.data.deleteManyUpworkJobsSearchCriteria.deletedCount)}`);
            ...
            if (criteria && criteria.length > 0) {
                ...
                const createManyUpworkJobsSearchCriteriaMutation = (0, core_1.gql) `
                ...
                console.log(`Create New Criteria result: ${JSON.stringify(createNewCriteriaResult.data.createManyUpworkJobsSearchCriteria)}`);
            }
            return true;
        }
        catch (error) {
    ...
    async getEmployeesJobPosts(data) {
        ...
        try {
            ...
            const employeesQuery = (0, core_1.gql) `
            ...
            do {
                ...
                const jobsResponse = result.data.employeeJobPosts.edges.map((it) => {
                    ...
                    const res = {
                        ...
                        jobStatus: rec.jobStatus ? contracts_1.JobPostStatusEnum[rec.jobStatus] : undefined,
                        jobType: rec.jobType ? contracts_1.JobPostTypeEnum[rec.jobType] : undefined,
    ...
    async getEmployeeJobPostId(employeeId, jobPostId) {
        const employeeJobPostsQuery = (0, core_1.gql) `
    ...
    async getJobPostId(providerCode, providerJobId) {
        const jobPostsQuery = (0, core_1.gql) `
    ...
    init() {
        try {
            const gauzyAIRESTEndpoint = this._configService.get('gauzyAI.gauzyAIRESTEndpoint');
            console.log(chalk.magenta(`GauzyAI REST Endpoint: ${gauzyAIRESTEndpoint}`));
            this.gauzyAIGraphQLEndpoint = this._configService.get('gauzyAI.gauzyAIGraphQLEndpoint');
    ...
    async syncEmployee(employee) {
        ...
        try {
            ...
            let employeesQuery = (0, core_1.gql) `
				query employeeByExternalEmployeeId($externalEmployeeIdFilter: String!) {
					employees(filter: { externalEmployeeId: { eq: $externalEmployeeIdFilter } }) {
						edges {
							node {
            ...
            if (!isAlreadyCreated) {
                ...
                if (!isAlreadyCreated) {
                    const createEmployeeMutation = (0, core_1.gql) `
    ...
    async syncUser(user) {
        ...
        let userFilterByExternalFieldsQuery = (0, core_1.gql) `
		...
		`;
        let usersQueryResult = await this._client.query({
        ...
        try {
            ...
            if (!isAlreadyCreated) {
                ...
                try {
                    const createOneUserMutation = (0, core_1.gql) `
                    ...
                    return newUser.data.createOneUser;
                }
                catch (error) {
            ...
            try {
                ...
                const updateUserMutation = (0, core_1.gql) `
                ...
                return updateUserResponse.data.updateOneUser;
            }
            catch (error) {
     ...
     */
    async updateOneTenantApiKey(input) {
        // Search for the tenant API key by its external API key
        let tenantApiKeyFilterByExternalFieldsQuery = (0, core_1.gql) `
		...
		`;
        let tenantApiKeysQueryResult = await this._client.query({
        ...
        try {
            ...
            try {
                ...
                const updateOneTenantApiKeyMutation = (0, core_1.gql) `
                ...
                return updateOneTenantApiKeyResponse.data.updateOneTenantApiKey;
            }
            catch (error) {
     ...
     */
    async getTenantByExternalTenantId(externalTenantId) {
        // Validate externalTenantId
        if (!externalTenantId) {
            throw new common_1.HttpException('External Tenant ID is required', common_1.HttpStatus.BAD_REQUEST);
        }
        // Define the GraphQL query outside the function
        const tenantByExternalTenantIdQuery = (0, core_1.gql) `
        ...
        try {
            ...
            const tenantsQueryResult = await this._client.query({
...
exports.GauzyAIService = GauzyAIService = GauzyAIService_1 = tslib_1.__decorate([
