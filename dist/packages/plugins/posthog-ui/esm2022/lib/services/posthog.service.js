...
export class PostHogService {
     ...
     */
    setupRouteTracking(router = this.router) {
        if (!isPlatformBrowser(this.platformId) || !this.initialized || this._routeTrackingAttached)
            return;
        this._routeTrackingAttached = true;
     ...
     */
    capturePageview(url, properties = {}) {
        if (!this.initialized)
            return;
     ...
     */
    captureEvent(eventName, properties = {}, sendInstantly) {
        if (!this.initialized)
            return;
        posthog.capture(eventName, properties, sendInstantly !== undefined ? { send_instantly: sendInstantly } : undefined);
    }
    /**
     ...
     */
    identify(distinctId, userProperties = {}, callback) {
        if (!this.initialized)
            return;
     ...
     * @param callback - Optional callback function
     */
    alias(alias, distinctId, callback) {
        if (!this.initialized)
            return;
     ...
     */
    getFeatureFlag(key, defaultValue = null) {
        if (!this.initialized)
            return defaultValue;
     ...
     * @param callback - Optional callback function
     */
    setPersonProperties(properties, value, callback) {
        if (!this.initialized)
            return;
     ...
     * @param callback - Optional callback function
     */
    setPersonPropertiesOnce(properties, value, callback) {
        if (!this.initialized)
            return;
     ...
     */
    setGroupPropertiesForFlags(groupTypeToPropertiesMap, reloadFeatureFlags) {
        if (!this.initialized)
            return;
     ...
     */
    captureDeadClick(element, properties = {}) {
        if (!this.initialized)
            return;
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: PostHogService, deps: [{ token: i1.Router }, { token: PLATFORM_ID }], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: PostHogService, providedIn: 'root' }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: PostHogService, decorators: [{
            type: Injectable,
            args: [{
                    providedIn: 'root'
                }]
