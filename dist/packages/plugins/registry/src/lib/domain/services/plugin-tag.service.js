...
exports.PluginTagService = void 0;
const tslib_1 = require("tslib");
const core_1 = require("@gauzy/core");
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
const mikro_orm_plugin_tag_repository_1 = require("../repositories/mikro-orm-plugin-tag.repository");
const type_orm_plugin_tag_repository_1 = require("../repositories/type-orm-plugin-tag.repository");
/**
 * PluginTag Service
 *
 ...
 */
let PluginTagService = class PluginTagService extends core_1.TenantAwareCrudService {
    constructor(typeOrmPluginTagRepository, mikroOrmPluginTagRepository) {
        super(typeOrmPluginTagRepository, mikroOrmPluginTagRepository);
        this.typeOrmPluginTagRepository = typeOrmPluginTagRepository;
        this.mikroOrmPluginTagRepository = mikroOrmPluginTagRepository;
    }
    /**
    ...
    async createTag(entity) {
        ...
        if (existingRelation) {
            throw new common_1.BadRequestException(`Plugin-tag relationship already exists for plugin ${entity.pluginId} and tag ${entity.tagId}`);
        }
        // Add current tenant/organization context if not provided
        const createData = {
            ...entity,
            tenantId: entity.tenantId || core_1.RequestContext.currentTenantId(),
            organizationId: entity.organizationId || core_1.RequestContext.currentOrganizationId(),
            appliedAt: new Date()
        };
    ...
    async bulkDelete(input) {
        ...
        if (input.ids && input.ids.length > 0) {
            whereConditions.id = (0, typeorm_1.In)(input.ids);
        }
        if (input.tagIds && input.tagIds.length > 0 && input.pluginId) {
            whereConditions.tagId = (0, typeorm_1.In)(input.tagIds);
            whereConditions.pluginId = input.pluginId;
        }
        ...
        switch (this.ormType) {
            case core_1.MultiORMEnum.MikroORM: {
                const count = await this.mikroOrmRepository.nativeDelete(whereConditions);
                return count;
            }
     ...
     * @returns Promise<IPluginTag[]> - Updated plugin-tag relationships
     */
    async updateTagsPriority(priorities) {
        const updatedRelationships = [];
        for (const priorityUpdate of priorities) {
            try {
                // Update the priority field (assuming it exists in the entity)
                await this.update(priorityUpdate.id, { priority: priorityUpdate.priority });
    ...
    async getStatistics(tenantId, organizationId) {
        ...
        switch (this.ormType) {
            case core_1.MultiORMEnum.MikroORM: {
                ...
                if (tenantId)
                    taggedPluginsQb = taggedPluginsQb.where('tenantId', tenantId);
                if (organizationId)
                    taggedPluginsQb = taggedPluginsQb.andWhere('organizationId', organizationId);
                ...
                if (tenantId)
                    usedTagsQb = usedTagsQb.where('tenantId', tenantId);
                if (organizationId)
                    usedTagsQb = usedTagsQb.andWhere('organizationId', organizationId);
                ...
                let popularTagsQb = knex('plugin_tags as pt')
                    ...
                    .limit(10);
                if (tenantId)
                    popularTagsQb = popularTagsQb.where('pt.tenantId', tenantId);
                if (organizationId)
                    popularTagsQb = popularTagsQb.andWhere('pt.organizationId', organizationId);
                const popularTagsResults = await popularTagsQb;
                const popularTags = popularTagsResults.map((r) => ({
                    tagId: r.tagId,
                    tagName: r.tagName,
                ...
                let mostTaggedQb = knex('plugin_tags as pt')
                    ...
                    .limit(10);
                if (tenantId)
                    mostTaggedQb = mostTaggedQb.where('pt.tenantId', tenantId);
                if (organizationId)
                    mostTaggedQb = mostTaggedQb.andWhere('pt.organizationId', organizationId);
                const mostTaggedResults = await mostTaggedQb;
                const mostTaggedPlugins = mostTaggedResults.map((r) => ({
                    pluginId: r.pluginId,
            ...
            default: {
                ...
                if (tenantId)
                    taggedPluginsQuery.where('pluginTag.tenantId = :tenantId', { tenantId });
                if (organizationId)
                ...
                if (tenantId)
                    usedTagsQuery.where('pluginTag.tenantId = :tenantId', { tenantId });
                if (organizationId)
                    usedTagsQuery.andWhere('pluginTag.organizationId = :organizationId', { organizationId });
                ...
                const popularTagsQuery = this.typeOrmPluginTagRepository
                    ...
                    .limit(10);
                if (tenantId)
                    popularTagsQuery.where('pluginTag.tenantId = :tenantId', { tenantId });
                if (organizationId)
                    popularTagsQuery.andWhere('pluginTag.organizationId = :organizationId', { organizationId });
                ...
                const mostTaggedPluginsQuery = this.typeOrmPluginTagRepository
                    ...
                    .limit(10);
                if (tenantId)
                    mostTaggedPluginsQuery.where('pluginTag.tenantId = :tenantId', { tenantId });
                if (organizationId)
     ...
     * @returns Promise<IPluginTag[]>
     */
    async replacePluginTags(pluginId, tagIds, tenantId, organizationId) {
     ...
     */
    async findSimilarPlugins(pluginId, limit = 10) {
        switch (this.ormType) {
            case core_1.MultiORMEnum.MikroORM: {
                const knex = this.mikroOrmRepository.getKnex();
     ...
     */
    async validateRelationship(pluginId, tagId) {
...
exports.PluginTagService = PluginTagService;
exports.PluginTagService = PluginTagService = tslib_1.__decorate([
    (0, common_1.Injectable)(),
    tslib_1.__metadata("design:paramtypes", [type_orm_plugin_tag_repository_1.TypeOrmPluginTagRepository,
        mikro_orm_plugin_tag_repository_1.MikroOrmPluginTagRepository])
], PluginTagService);
//# sourceMappingURL=plugin-tag.service.js.map