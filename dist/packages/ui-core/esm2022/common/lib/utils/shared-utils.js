 ...
 */
export function isNotNullOrUndefinedOrEmpty(value) {
 ...
 */
export function convertPrecisionFloatDigit(val, digit = 6) {
    return parseFloat(parseFloat(val.toString()).toFixed(digit));
}
/*
 * Retrieve name from email address
 */
export function retrieveNameFromEmail(email) {
    if (email) {
...
export function convertLocalToTimezone(localDt, localDtFormat, timeZone, format = 'YYYY-MM-DD hh:mm:ss') {
    return timezone(localDt, localDtFormat).tz(timeZone).format(format);
}
/*
 * Capitalize the first letter of a string being
 */
export function ucFirst(str, force) {
 ...
 */
export function getUTCOffsetForTimezone(timezone = 'UTC') {
 ...
 */
export function toUtcOffset(date, timezone) {
 ...
 * @returns An operator function that filters out consecutive duplicate values in an observable sequence.
 */
export function distinctUntilChange() {
    return (source) => {
 ...
 */
export function employeeMapper(row) {
    return {
 ...
 * @returns
 */
export function __prepareExternalUrlLocation(url) {
 ...
 * @returns {string}
 */
export function sluggable(string, replacement = '-') {
 ...
 */
export function compressImage(base64Image, width, height) {
 ...
 */
export function removeDuplicatesByProperty(items, property) {
 ...
 */
export function validateUniqueString(existingStrings, value) {
