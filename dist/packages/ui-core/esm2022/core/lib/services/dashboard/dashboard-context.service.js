 ...
 */
export const DASHBOARD_TIME_ZONE_SOURCE = new InjectionToken('DASHBOARD_TIME_ZONE_SOURCE');
/**
 ...
 */
export function dashboardContextKey(context) {
    return [
        context.tenantId,
 ...
 */
export function narrowDashboardContext(context, overrides) {
    if (!overrides) {
        return context;
    }
 ...
 */
function adjustDateRangeFutureAllowed(request) {
    const now = moment();
...
export class DashboardContextService {
    constructor() {
        ...
        this._dateRangePickerBuilderService = inject(DateRangePickerBuilderService);
        this._timeZoneSource = inject(DASHBOARD_TIME_ZONE_SOURCE, { optional: true });
        /** Last published context, or `null` before the first subscriber. */
        this._snapshot = null;
        this._timeZone$ = this._timeZoneSource
            ? this._timeZoneSource.timeZone$
            : this._defaultTimeZone$();
        this._timeFormat$ = this._timeZoneSource
        ...
        filter(([organization, dateRange]) => !!organization && !!dateRange), debounceTime(CONTEXT_SETTLE_MS), map(([organization, dateRange, employee, project, team, timeZone, timeFormat]) => this._buildContext(organization, dateRange, employee, project, team, timeZone, timeFormat)), distinctUntilChanged((previous, current) => dashboardContextKey(previous) === dashboardContextKey(current)), tap((context) => (this._snapshot = context)), shareReplay({ bufferSize: 1, refCount: false }));
    }
    /**
    ...
    contextFor(overrides) {
        ...
        return this.context$.pipe(map((context) => narrowDashboardContext(context, overrides)), distinctUntilChanged((previous, current) => dashboardContextKey(previous) === dashboardContextKey(current)), shareReplay({ bufferSize: 1, refCount: true }));
    }
    /**
     ...
     */
    _buildContext(organization, dateRange, employee, project, team, timeZone, timeFormat) {
    ...
    _defaultTimeZone$() {
        ...
        ]).pipe(map(([organization, user]) => this._canChangeSelectedEmployee()
            ? organization?.timeZone || DEFAULT_DASHBOARD_TIME_ZONE
            : user?.timeZone || moment.tz.guess()), distinctUntilChanged());
    }
    /**
    ...
    _canChangeSelectedEmployee() {
        return this._store.hasPermission(PermissionsEnum.CHANGE_SELECTED_EMPLOYEE);
    }
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DashboardContextService, deps: [], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DashboardContextService, providedIn: 'root' }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DashboardContextService, decorators: [{
            type: Injectable,
            args: [{ providedIn: 'root' }]
        }] });
//# sourceMappingURL=dashboard-context.service.js.map