...
export function isLayoutV2(layout) {
    return !!layout && layout.version === 2 && Array.isArray(layout.tabs);
}
/**
 ...
 */
export function normalizeLayout(layout, defaultTabName = 'Overview') {
    if (isLayoutV2(layout)) {
        const tabs = [...layout.tabs]
            ...
            widgets: packLayout((Array.isArray(tab.widgets) ? tab.widgets : []).filter((placement) => !!placement && typeof placement === 'object' && !!placement.widgetId))
        }))
...
/** Builds an empty tab. */
export function emptyTab(name = 'Overview') {
 ...
 */
export function packLayout(placements) {
 ...
 */
export function addPlacement(placements, placement) {
 ...
 */
export function movePlacement(placements, fromIndex, toIndex) {
...
export function removePlacement(placements, instanceId) {
...
export function resizePlacement(placements, instanceId, size) {
    return packLayout((placements ?? []).map((placement) => placement.instanceId === instanceId
        ? clampPlacement({ ...placement, w: size.w ?? placement.w, h: size.h ?? placement.h })
        : placement));
}
//# sourceMappingURL=dashboard-layout.utils.js.map