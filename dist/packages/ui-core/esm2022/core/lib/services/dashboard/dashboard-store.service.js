 ...
 */
let DashboardStoreService = class DashboardStoreService {
    constructor(_dashboardService, _store, _router) {
        this._dashboardService = _dashboardService;
        this._store = _store;
        this._router = _router;
        this._dashboards$ = new BehaviorSubject([]);
        /** The current user's custom dashboards (for the selected organization). */
        this.dashboards$ = this._dashboards$.asObservable();
        this._selectedDashboard$ = new BehaviorSubject(null);
        /** The currently applied custom dashboard, or `null` when the Standard dashboard is active. */
        this.selectedDashboard$ = this._selectedDashboard$.asObservable();
    ...
    |--------------------------------------------------------------------------
    */
    _listenToChangesAndLoadDashboards() {
        combineLatest([
        ...
        switchMap(([organization]) => {
            ...
            return from(this._loadDashboards()).pipe(catchError((error) => {
     ...
     */
    _leaveOrganization(organizationId) {
        ...
        if (this.selectedDashboard || localStorage.getItem(selectedKey)) {
            let layout = {};
            try {
                layout = JSON.parse(localStorage.getItem(backupKey) || '{}');
            }
            catch {
                layout = {};
            }
            this._store.widgets = (Array.isArray(layout.widgets) ? layout.widgets : []);
            this._store.windows = (Array.isArray(layout.windows) ? layout.windows : []);
        }
    ...
    async _loadDashboards() {
        ...
        return items.filter((item) => this._isLayoutDashboard(item));
    }
    /**
    ...
    */
     ...
     */
    async selectById(id) {
        let dashboard = this.dashboards.find((item) => item.id === id);
        if (!dashboard) {
            ...
            this._dashboards$.next(items);
            dashboard = items.find((item) => item.id === id);
        }
        if (!dashboard) {
        ...
        localStorage.setItem(this._selectedKey, dashboard.id);
    ...
    async resolveDefaultDashboard() {
        ...
        const loaded = this.dashboards.find((item) => item.isDefault && item.organizationId === organizationId);
        if (loaded) {
            return loaded;
        }
        ...
        return items.find((item) => item.isDefault && this._isLayoutDashboard(item)) ?? null;
    }
    /*
    |--------------------------------------------------------------------------
    | Navigation helpers
    ...
    */
     ...
     */
    async createDashboard(name, layout = null) {
        const { id: organizationId, tenantId } = this._store.selectedOrganization || {};
        const dashboard = await this._dashboardService.create({
            ...
            contentHtml: (layout ?? normalizeLayout({})),
            organizationId,
            tenantId,
     ...
     */
    async duplicateDashboard(source, name) {
        if (!source) {
    ...
    async clearDefaultDashboard() {
        const defaults = this.dashboards.filter((item) => item.isDefault);
        await Promise.all(defaults.map((item) => this._dashboardService.update(item.id, { isDefault: false })));
        this.refresh();
    }
    /**
    ...
    */
    ...
    async saveSelectedLayout() {
        ...
        const updated = await this._dashboardService.update(selected.id, {
            contentHtml: this.captureLayout()
        });
        this._editing$.next(false);
    ...
    */
     ...
     */
    async saveLayoutV2(dashboardId, layout) {
        ...
        if (selected?.id === dashboardId) {
            this._selectedDashboard$.next({
                ...selected,
                contentHtml: (updated?.contentHtml ?? normalized)
            });
        }
        this.refresh();
        return updated;
    ...
    _cloneLayoutV2(layout) {
        return {
            ...
            tabs: (layout.tabs ?? []).map((tab) => ({
                ...tab,
                id: createId(),
                widgets: (tab.widgets ?? []).map((placement) => ({
                    ...placement,
                    instanceId: createId(),
    ...
    */
    ...
    _sanitizeLayoutItems(items) {
        return (items ?? [])
            .filter((item) => !!item && typeof item.position === 'number')
            .map((item) => ({
    ...
    _restoreStandardSnapshot() {
        let layout = {};
        try {
            layout = JSON.parse(localStorage.getItem(this._backupKey) || '{}');
        }
        catch {
            layout = {};
        }
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DashboardStoreService, deps: [{ token: i1.DashboardService }, { token: i2.Store }, { token: i3.Router }], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DashboardStoreService, providedIn: 'root' }); }
};
...
export { DashboardStoreService };
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: DashboardStoreService, decorators: [{
            type: Injectable,
            args: [{
                    providedIn: 'root'
                }]
        }], ctorParameters: () => [{ type: i1.DashboardService }, { type: i2.Store }, { type: i3.Router }] });
