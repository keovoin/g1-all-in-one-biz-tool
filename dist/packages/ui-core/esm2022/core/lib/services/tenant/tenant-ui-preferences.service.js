...
export class TenantUiPreferencesService {
    constructor() {
        ...
        this.API_URL = `${API_PREFIX}/tenant-ui-preferences`;
        ...
        this.preferredUi = signal(PreferredUiEnum.ANGULAR, ...(ngDevMode ? [{ debugName: "preferredUi" }] : []));
        this.preferredUi$ = toObservable(this.preferredUi);
        /** `true` when the tenant asked for the React flavour. */
        this.isReact = computed(() => this.preferredUi() === PreferredUiEnum.REACT, ...(ngDevMode ? [{ debugName: "isReact" }] : []));
    ...
    ensureLoaded() {
        ...
        const promise = firstValueFrom(this.http.get(this.API_URL))
            .then((preferences) => {
    ...
    async update(input) {
        const tenantId = this.currentTenantId();
        const preferences = await firstValueFrom(this.http.put(this.API_URL, input));
        if (this.currentTenantId() === tenantId) {
            this.loadedForTenantId = tenantId;
            this.apply(tenantId, preferences?.preferredUi);
        }
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: TenantUiPreferencesService, deps: [], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: TenantUiPreferencesService, providedIn: 'root' }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: TenantUiPreferencesService, decorators: [{
            type: Injectable,
...
function readStoredPreferredUi(tenantId) {
    try {
...
function isPreferredUi(value) {
    return Object.values(PreferredUiEnum).includes(value);
}
//# sourceMappingURL=tenant-ui-preferences.service.js.map