...
import { computed, DEFAULT_CURRENCY_CODE, Directive, inject, LOCALE_ID, signal } from '@angular/core';
 ...
 */
export class BaseAccountingWidgetComponent extends BaseDashboardWidgetComponent {
    constructor() {
        super(...arguments);
        this._statisticsCache = inject(AccountingStatisticsCacheService);
        /**
         * Currency formatter.
         *
         ...
         */
        this._currencyPipe = new CurrencyPipe(inject(LOCALE_ID), inject(DEFAULT_CURRENCY_CODE));
        ...
        this.total = computed(() => this.statistics()?.total ?? null, ...(ngDevMode ? [{ debugName: "total" }] : []));
        /**
         ...
         */
        this.currency = computed(() => this.context()?.organization?.currency || this.context()?.currency || undefined, ...(ngDevMode ? [{ debugName: "currency" }] : []));
        /** `LEFT` / `RIGHT` placement of the currency symbol, as configured per organization. */
        this.currencyPosition = computed(() => this.context()?.organization?.currencyPosition || undefined, ...(ngDevMode ? [{ debugName: "currencyPosition" }] : []));
        /**
         ...
         */
        this.hasBonusType = computed(() => !!this.context()?.organization?.bonusType, ...(ngDevMode ? [{ debugName: "hasBonusType" }] : []));
    }
    /**
    ...
    observeStatistics() {
        ...
        filter((context) => !!context?.organizationId), distinctUntilChanged((previous, current) => previous.tenantId === current.tenantId &&
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: BaseAccountingWidgetComponent, deps: null, target: i0.ɵɵFactoryTarget.Directive }); }
    static { this.ɵdir = i0.ɵɵngDeclareDirective({ minVersion: "14.0.0", version: "21.0.7", type: BaseAccountingWidgetComponent, isStandalone: true, usesInheritance: true, ngImport: i0 }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: BaseAccountingWidgetComponent, decorators: [{
            type: Directive
        }] });
/**
 ...
 */
function isSameInstant(previous, current) {
...
//# sourceMappingURL=base-accounting-widget.component.js.map