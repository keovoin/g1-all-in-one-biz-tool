 ...
 * users already know.
 */
export const CASH_FLOW_SERIES_KEYS = {
...
export const DEFAULT_CASH_FLOW_LABELS = {
 ...
 */
export function buildCashFlowChartData(rows, palette, labels) {
    const chart = Array.isArray(rows) ? rows : [];
    return {
        labels: chart.map((row) => row?.dates ?? ''),
        datasets: [
 ...
 */
function buildDataset(label, rows, key, color) {
    return {
        label,
        data: rows.map((row) => Number(row?.statistics?.[key]) || 0),
        borderColor: color,
...
//# sourceMappingURL=cash-flow-chart.utils.js.map