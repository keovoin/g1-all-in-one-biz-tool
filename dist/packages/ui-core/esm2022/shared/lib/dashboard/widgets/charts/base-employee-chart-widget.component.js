...
import { EmployeeChartKind, buildDoughnutChartData, buildDoughnutChartOptions, buildHorizontalBarChartData, buildHorizontalBarChartOptions, buildStackedBarChartData, buildStackedBarChartOptions, formatEmployeeCurrency, resolveEmployeeChartPalette, toChartType } from './employee-chart.utils';
 ...
 */
export class BaseEmployeeChartWidgetComponent extends BaseDashboardWidgetComponent {
    constructor() {
        super(...arguments);
        ...
        this._currencyPositionPipe = inject(CurrencyPositionPipe);
        ...
        this.statistics = signal([], ...(ngDevMode ? [{ debugName: "statistics" }] : []));
        ...
        this.seriesLabels = signal(DEFAULT_SERIES_LABELS, ...(ngDevMode ? [{ debugName: "seriesLabels" }] : []));
        /** The employee the charts describe, or `null` when none is selected. */
        this.employeeId = computed(() => this.widgetContext()?.employeeIds?.[0] ?? null, ...(ngDevMode ? [{ debugName: "employeeId" }] : []));
        /**
        ...
        this.isEmpty = computed(() => !this.statistics().length, ...(ngDevMode ? [{ debugName: "isEmpty" }] : []));
        /** Everything the dataset builders need beyond the raw statistics. */
        this.chartContext = computed(() => {
            ...
            return {
                palette,
                labels,
                formatCurrency: (value) => formatEmployeeCurrency(value, organization, this._currencyPipe, this._currencyPositionPipe)
            };
        }, ...(ngDevMode ? [{ debugName: "chartContext" }] : []));
    }
    /**
    ...
    observeSeriesLabels() {
        combineLatest([
            ...
            .pipe(map(([revenue, expenses, profit, bonus]) => ({ revenue, expenses, profit, bonus })), tap((labels) => this.seriesLabels.set(labels)), takeUntilDestroyed(this.destroyRef))
            .subscribe();
    }
    /**
    ...
    observeStatistics() {
        ...
        const scopedContext$ = this.context$.pipe(
        // A context without an organization cannot produce a meaningful request;
        // the canvas shows the "select an organization" state.
        filter((context) => !!context?.organizationId), distinctUntilChanged((previous, current) => statisticsScopeKey(previous) === statisticsScopeKey(current)));
        ...
        switchMap((context) => this.fetchStatistics(context)), tap((statistics) => {
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: BaseEmployeeChartWidgetComponent, deps: null, target: i0.ɵɵFactoryTarget.Directive }); }
    static { this.ɵdir = i0.ɵɵngDeclareDirective({ minVersion: "14.0.0", version: "21.0.7", type: BaseEmployeeChartWidgetComponent, isStandalone: true, usesInheritance: true, ngImport: i0 }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: BaseEmployeeChartWidgetComponent, decorators: [{
            type: Directive
        }] });
/**
 ...
 */
function statisticsScopeKey(context) {
    return [
        context.tenantId ?? '',
...
//# sourceMappingURL=base-employee-chart-widget.component.js.map