 ...
 */
export function buildMonthStatisticsRequest(context, employeeId) {
 ...
 */
function monthStatisticsCacheKey(request) {
    return [
...
export class EmployeeMonthStatisticsCacheService {
     ...
     */
    getMonthStatistics(context, employeeId) {
        const request = buildMonthStatisticsRequest(context, employeeId);
        const key = monthStatisticsCacheKey(request);
        return this._invalidated$.pipe(startWith(undefined), switchMap(() => this._cached(key, request)), 
    ...
    invalidate(context, employeeId) {
        const scope = context && employeeId ? monthStatisticsCacheKey(buildMonthStatisticsRequest(context, employeeId)) : '*';
        const now = Date.now();
    ...
    _cached(key, request) {
        ...
        const own = {};
        const stream$ = defer(() => from(this._employeeStatisticsService.getAggregatedStatisticsByEmployeeId(request))).pipe(shareReplay({ bufferSize: 1, refCount: false }), catchError((error) => {
    ...
    static { this.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: EmployeeMonthStatisticsCacheService, deps: [], target: i0.ɵɵFactoryTarget.Injectable }); }
    static { this.ɵprov = i0.ɵɵngDeclareInjectable({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: EmployeeMonthStatisticsCacheService, providedIn: 'root' }); }
}
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "21.0.7", ngImport: i0, type: EmployeeMonthStatisticsCacheService, decorators: [{
            type: Injectable,
            args: [{ providedIn: 'root' }]
        }] });
//# sourceMappingURL=employee-month-statistics-cache.service.js.map