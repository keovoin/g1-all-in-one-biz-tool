 ...
 */
export function projectManagementScopeKey(context) {
    if (!context) {
        return '';
    }
    return [
 ...
 */
export function sortProjectsByPopularity(tasks) {
 ...
 */
export function openTasksMostRecentFirst(tasks) {
    ...
    return (tasks ?? []).filter((task) => task?.status === TaskStatusEnum.OPEN).reverse();
}
/**
 ...
 */
export function buildProjectManagementSnapshot(tasks, total) {
    const items = tasks ?? [];
    return {
        tasks: items,
...
//# sourceMappingURL=project-management-widget.utils.js.map