...
export async function generatePdf(invoice, organization, organizationContact, service, translatedText) {
    const body = [];
    for (const item of invoice.invoiceItems) {
        let currentItem = [];
        switch (invoice.invoiceType) {
            case InvoiceTypeEnum.BY_EMPLOYEE_HOURS:
                const employee = await service.getEmployeeById(item.employeeId, ['user']);
                currentItem = [
    ...
    const docDefinition = {
        content: [
                ...
                columns: [
                    {
                        ...
                        text: `${invoice.isEstimate
                            ? translatedText.estimate
                            : translatedText.invoice} ${translatedText.date}: ${invoice.invoiceDate.toString().slice(0, 10)}`
                    },
                    {
                        width: '50%',
            ...
            ' ',
            ' ',
            `${translatedText.paid}: ${invoice.paid ? translatedText.yes : translatedText.no}`,
            ' ',
            ' ',
