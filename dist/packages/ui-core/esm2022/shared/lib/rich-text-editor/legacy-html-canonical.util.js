...
export function canonicalColor(value) {
    return value.replace(/rgb\(\s*(\d+)[,\s]+(\d+)[,\s]+(\d+)\s*\)/gi, (_match, r, g, b) => `#${[r, g, b].map((part) => Number(part).toString(16).padStart(2, '0')).join('')}`);
}
/**
...
export function canonicalStyle(style) {
    return (style
        ...
        const value = canonicalColor(declaration
            .slice(separator + 1)
            .trim()
            .toLowerCase());
...
export const serializeBody = (body) => Array.from(body.childNodes).map(serialize).join('');
/**
...
//# sourceMappingURL=legacy-html-canonical.util.js.map