...
/**
 * Sastra AI provider definition for the AI chat engine (placeholder).
 *
 ...
 * routed through Sastra AI — {@link IAiChatProviderDefinition.createModel}
 * intentionally throws until the Sastra AI chat proxy is implemented
...
export const gauzyAiProviderDefinition: IAiChatProviderDefinition = {
	 ...
	 * tenant-first, so a tenant that saves a Sastra AI key through the BYOK settings page would still
	...
	/**
	 * Not implemented yet — Sastra AI chat routing is a planned integration.
	 *
	 * @param _modelId Ignored.
	 * @param _credentials Ignored.
	 * @throws Always throws until chat requests can be proxied to Sastra AI.
	 */
	async createModel(_modelId: string, _credentials: IAiProviderCredentials): Promise<never> {
		throw new Error(
			'Chat via Sastra AI is not implemented yet — configure another provider. See @gauzy/plugin-ai-provider-gauzy-ai README.'
		);
	}
};
