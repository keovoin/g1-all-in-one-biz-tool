...
export class IntegrationAIAnalysisService {
	...
	/**
	 * Analyze screenshot using Sastra AI service.
	 *
	...
	async analyzeAndSaveScreenshot(event: ScreenshotEvent) {
		...
		// Analyze image using Sastra AI service
	...
	/**
	 * Analyze an image using Sastra AI service.
		...
		try {
			...
			if (!!integration) {
				try {
					...
					console.log('Error while getting Integration for Sastra AI', error.message);
					return null;
				}
			}
