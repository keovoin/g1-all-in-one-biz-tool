	...
	async use(request: Request, _response: Response, next: NextFunction) {
		try {
			...
			if (isNotEmpty(tenantId) && isNotEmpty(organizationId)) {
				if (this.logging) {
					console.log(
						`Getting Sastra AI integration settings from Cache for tenantId: ${tenantId}, organizationId: ${organizationId}`
					);
				}
				...
				if (!integrationTenantSettings) {
					if (this.logging) {
						console.log(
							`Sastra AI integration settings NOT loaded from Cache for tenantId: ${tenantId}, organizationId: ${organizationId}`
						);
					}
					...
					if (fromDb && fromDb.settings) {
						...
						if (this.logging) {
							console.log(
								`Sastra AI integration settings loaded from DB and stored in Cache for tenantId: ${tenantId}, organizationId: ${organizationId}`
							);
						}
					}
				} else {
					if (this.logging) {
						console.log(
							`Sastra AI integration settings loaded from Cache for tenantId: ${tenantId}, organizationId: ${organizationId}`
						);
					}
				}
