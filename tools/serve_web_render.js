// Sastra Solution — Render static web server + /api proxy (HTTP + WebSocket) + SPA fallback
// Serves dist/apps/gauzy on $PORT (Render injects it); any non-file path falls back to index.html.
// Injects a secure-context polyfill (crypto.randomUUID/getRandomValues) into index.html.
...
// WebSocket passthrough (socket.io realtime) -> API
server.on('upgrade', (req, socket, head) => {
  const conn = net.connect(API.port, API.host, () => {
    const reqLine = (req.method || 'GET') + ' ' + (req.url || '/') + ' HTTP/1.1\r\n';
    let headers = reqLine;
    const raw = req.rawHeaders || [];
    for (let i = 0; i < raw.length; i += 2) {
      const k = raw[i], v = raw[i + 1];
      if (k.toLowerCase() === 'host') continue;
      headers += k + ': ' + v + '\r\n';
    }
    headers += 'Host: ' + API.host + ':' + API.port + '\r\n\r\n';
...
server.listen(PORT, '0.0.0.0', () => {
  console.log('Sastra web serving ' + ROOT + ' on http://0.0.0.0:' + PORT + ' (proxy /api -> ' + API.port + ')');
});
